...
```
每条超边的消息数 = |premises| + |conclusions| ≈ 4 (平均)
...
```
P(x₁, x₂, ..., xₙ) ∝ ∏ᵢ priorᵢ(xᵢ) × ∏ⱼ factorⱼ(x_premises, x_conclusions)
```
...
```
传统布局（当前实现）：
  factors = [{"edge_id": 1, "premises": [1,2], "conclusions": [3], "prob": 0.9}, ...]
  → Python dict，内存不连续，cache miss 严重

优化布局（CSR-like 压缩格式）：
  factor_premises:  [1, 2, | 3, 4, 5, | ...]     ← 连续数组
  premise_offsets:  [0,     2,         5,   ...]   ← 每个因子的 premises 起始位置
  factor_conclusions:  [3, | 6, 7, | ...]             ← 连续数组
  conclusion_offsets:  [0,  1,      3,   ...]
...
如果两条边的局部拓扑结构相同（2-premise, 1-conclusion, 相同 probability），
