...
| 变更记录 | v1.1: 按 API v2 重构 §3/§5/§7; v1.2: 存储架构重构为 Neo4j+LanceDB+ByteHouse 三层，去除 DuckDB 和 Redis; v1.3: 与模块详细设计对齐——移除 resolve 步骤和 merge_propositions 操作，统一命名 (/nodes, /hyperedges)，节点状态简化为 active/deleted |
...
3. **构建管线**：在 10^11 量级候选对中筛选有效关系，采用轻量级分类器预过滤 + LLM oracle 混合策略。外部论文注入通过 batch commit API 入图，内部知识发现（Join/Meet/Verify）由 merge 操作异步触发。
...
| 数据类别 | 数量 | 单条大小 | 总容量 |
|---------|------|---------|--------|
| **原始命题节点** (paper-extract) | 5 x 10^8 | ~2KB (content + metadata + notations) | ~1 TB |
| **Join 衍生命题节点** | 1 x 10^8 | ~1.5KB | ~150 GB |
| **Meet 衍生命题节点** (deduction + conjecture) | 5 x 10^8 | ~1.5KB | ~750 GB |
| **命题节点总计** | **10^9** | - | **~1.9 TB** |
...
| **LLM 调用速率** | 500 calls/秒（构建时峰值） | join/meet/verify + commit review |
...
- **内部构建管线**：Join/Meet/Verify 是系统内部的知识发现流程，由 merge 操作异步触发，不经过外部 commit API。
...
| **Internal Build Pipeline** | 知识发现（Join/Meet/Verify），merge 后异步触发 | Kafka + GPU 集群 + LLM API |
...
| **内部管线** | | |
| merge → Join/Meet/Verify 触发 | **异步 (Kafka)** | 解耦写入和知识发现 |
...
propositions_schema = pa.schema([
    ...
    pa.field("type", pa.utf8()),                         # paper-extract | join | deduction | conjecture
    pa.field("subtype", pa.utf8()),                      # subsumption | partial_overlap | ...
    pa.field("round", pa.int32()),                       # 创建轮次

    # 命题内容
    pa.field("text", pa.utf8()),                         # 命题文本（~500 chars avg）
    pa.field("notations", pa.utf8()),                    # LaTeX 符号表示
    pa.field("assumptions", pa.list_(pa.utf8())),        # 前提假设列表
    pa.field("keywords", pa.list_(pa.utf8())),           # 关键词列表

    # 来源信息
    pa.field("paper_id", pa.utf8()),                     # 关联论文 ID
    pa.field("section", pa.utf8()),                      # 论文章节
    pa.field("confidence", pa.float32()),                # LLM 提取置信度
...
| text + notations + assumptions + keywords | ~1.5KB | ~1.5 TB | 变长文本，Lance 使用字典编码压缩 |
| paper_id + section + confidence + belief + timestamps | ~110B | ~110 GB | 固定/短字段 |
...
Meet (多个前提 → 一个结论):
  ...
  (:Proposition {id:"A"}) -[:TAIL]→ (:Hyperedge:Meet {id:"e1", commit_id:"c1"}) -[:HEAD]→ (:Proposition {id:"D"})
  (:Proposition {id:"B"}) -[:TAIL]→ (:Hyperedge:Meet ...)
  (:Proposition {id:"C"}) -[:TAIL]→ (:Hyperedge:Meet ...)


Join (一个弱命题 → 多个强命题):

  (:Proposition {id:"A"}) -[:TAIL]→ (:Hyperedge:Join {id:"e2", commit_id:"c2"}) -[:HEAD]→ (:Proposition {id:"B1"})
...
-- :Hyperedge 是通用标签，:Meet / :Join 是类型标签
CREATE CONSTRAINT FOR (e:Hyperedge) REQUIRE e.id IS UNIQUE;

-- 创建 Meet 超边示例
CREATE (e:Hyperedge:Meet {id: 'e1', commit_id: 'c1', probability: 0.95, verified: false, created_at: datetime()})
...
| :TAIL 关系 | ~10x10^9 | ~60B | ~600 GB | Meet 平均 3 条 + Join 1 条 |
| :HEAD 关系 | ~10x10^9 | ~60B | ~600 GB | Meet 1 条 + Join 平均 3 条 |
...
> **与 API v2 的对齐说明**：API v2 确立了 Git-like commit 写入模型，所有外部写入通过 commit → review → merge 工作流入图。本节描述的内部 Build Pipeline（Join/Meet/Verify）是系统自主的知识发现流程，由 merge 操作异步触发。
...
| **Join/Meet/Verify** | 系统内部 | ❌ 直接写入存储 | 可信的内部流程，无需外部 review |
...
当前原型实现了三种 Join 操作（`join-cc`, `join-cp`, `join-jj`），Phase 1 保持相同的语义，但需要适配分布式处理。
...
graph TD
    ...
    subgraph S3["Step 3: Graph 写入"]
        SUB["subsumption"] --> SAVE_E["save_edge(type=join,<br/>subtype=subsumption)"]
        PO["partial_overlap"] --> SAVE_NE["save_node(type=join) +<br/>save_edge(type=join)"]
    ...
    subgraph S5["Step 5: dag_shared_children (J-J) DAG 结构发现"]
        S5I["遍历已有 join 边"] --> S5P["找到共享子节点的父节点<br/>(纯图遍历, 无需 ANN)"]
    end
...
|-------|--------|---------|----------------|
| `join.clusters.cc` | 64 | `{cluster_id, node_ids: [...]}` | `join-cc-workers` (500 instances) |
| `join.clusters.cp` | 64 | `{anchor_id, candidate_ids: [...]}` | `join-cp-workers` (500 instances) |
| `join.results` | 32 | `{cluster_id, edges: [...], nodes: [...]}` | `graph-writer` (16 instances) |
| `join.errors` | 8 | `{cluster_id, error: "..."}` | `error-handler` (4 instances) |

### 5.6 Meet 流程详细设计

Meet 操作在 Join 构建的 DAG 上运行，收集每个父节点的"兄弟组"（共享同一父节点的子节点），分析它们之间的关系。

```mermaid
graph TD
    subgraph S1["Step 1: 兄弟组收集"]
        S1A["遍历所有 type=join 的超边"] --> S1B["按 parent 分组:<br/>parent_id to child_ids"]
        S1B --> S1C["过滤: 保留 >= 2 个兄弟的组<br/>预期组数: ~5 x 10^6"]
    end

    subgraph S2["Step 2: LLM 分析 (每组 1 次调用)"]
        S2A["Prompt: meet.md"] --> S2B["contradiction: 兄弟间矛盾"]
    ...
    subgraph S3["Step 3: Graph 写入"]
        ...
        S3B["deduction"] --> S3BW["save_node(type=deduction) +<br/>save_edge(type=meet)"]
        S3C["conjecture"] --> S3CW["save_node(type=conjecture) +<br/>save_edge(type=meet)"]
    end

    subgraph S4["Step 4: Embedding 增量更新"]
        S4A["新建 deduction/conjecture 节点<br/>计算 embedding"] --> S4B["写入 LanceDB<br/>(命题 + 向量统一存储)"]
        S4B --> S4C["触发下一轮 Join<br/>(新节点参与新聚类)"]
```

> **注**：§5.4 中的 "embedding 相似度过滤" 和 "ANN 搜索" 在 Phase 1 中由 ByteHouse 执行（而非 LanceDB DiskANN）。Meet 阶段产出的新节点写入三层存储（LanceDB + Neo4j + ByteHouse）。
    end
...
graph TD
    subgraph S1["Step 1: 查找未验证的边"]
        S1A["find_unverified_edges(join)"] --> S1R["join 边列表"]
        S1B["find_unverified_edges(meet)"] --> S1S["meet 边列表"]
    end

    subgraph S2["Step 2: 交叉验证 (每条边 1 次 LLM 调用)"]
        S2A["使用不同 LLM prompt<br/>重新评估推理关系"]
        S2B["verify-join: 验证<br/>subsumption/partial_overlap 有效性"]
        S2C["verify-meet: 验证<br/>deduction 正确性、conjecture 非平凡性"]
    end
...
**成本估算**：
- Join 边约 10^8 条 → 验证需 10^8 次 LLM 调用 → ¥3,000,000（估算，使用通义千问-plus 级别模型）
- Meet 边约 10^8 条 → 验证需 10^8 次 LLM 调用 → ¥3,000,000（估算）
- **优化**：采样验证 10% + 高置信度边自动 pass → 实际约 2x10^7 次调用 → **¥600,000**

### 5.8 Contradiction 检测流程

矛盾在两个阶段被发现：
1. **Join 阶段**：相似命题聚类后，LLM 判断发现矛盾（被动发现）
2. **Meet 阶段**：兄弟命题组合分析时，LLM 发现前提矛盾（主动推理）

存储方案：
- 矛盾记录在 Neo4j 中作为 `:Hyperedge:Contradiction` 节点（详见 4.2 节）
- 矛盾边在图拓扑中作为 type="contradiction" 的超边
- 经 verify-meet 确认后标记 confirmed=true
...
| `join.clusters.*` | 64 | Join 聚类任务 |
| `join.results` | 32 | Join 结果 |
| `meet.groups` | 32 | Meet 兄弟组任务 |
| `meet.results` | 32 | Meet 结果 |
...
def compute_novelty(q, graph, lancedb_index):
    ...
    # Step 3: 检查是否已被 join (已泛化)
    subsumed = False
    for n in neighbors:
        if has_join_edge(n, q, graph):  # q 被某个 join 节点 subsume
            subsumed = True
            break
...
| 矛盾关系图的连通分量 | 每次 meet 操作后 | Neo4j（Cypher 连通分量查询） | 自动管理 |
...
graph TD
    ...
    subgraph MERGE["POST /commits/{id}/merge"]
        ...
        C5 --> C6["异步：触发 Join/Meet pipeline<br/>(新节点参与知识发现)"]
    end
```
...
graph LR
    ...
    D -->|"100ms"| E["图拓扑遍历<br/>(检查 join subsumption)"]
...
| Join 分类 (Level 3) | 10^7 | 高端 (qwen-max) | ¥0.05 | ¥500,000 |
| Meet 分析 | 5 x 10^6 | 高端 (qwen-max) | ¥0.05 | ¥250,000 |
...
| Join Pipeline (kafka-based) | 后端组 | 处理 10^4 clusters end-to-end |
| Meet Pipeline (kafka-based) | 后端组 | 处理 10^3 sibling groups end-to-end |
...
### B.2 Join 超边的 Factor Function

对于 join 类型超边（parent $\to$ children）：
$$f_{\text{join}}(\text{parent}, c_1, ..., c_k) = \prod_{i=1}^{k} P(c_i \mid \text{parent})$$

含义：parent 为 true 蕴含所有 children 为 true（因为 parent 是更弱/更一般的命题）。

### B.3 Meet 超边的 Factor Function

对于 meet 类型超边（siblings $\to$ conclusion）：
$$f_{\text{meet}}(s_1, ..., s_k, \text{conclusion}) = P(\text{conclusion} \mid s_1, ..., s_k)$$
