...
Gaia 的超边 `HyperEdge(tail=[1,2,3], head=[4,5])` 天然表达"多个前提推出多个结论"，无需拆解。
...
1. **有向 (Directed)**：`tail[]` → `head[]` 表示推理方向，是 B-超图 (directed hypergraph)
2. **语义类型化**：每条超边有 `type` (paper-extract, join, meet, contradiction, retraction) 和完整的 `reasoning` 推理链
...
Gaia 的节点类型 (`paper-extract`, `deduction`, `conjecture`, `join`) 显式编码了命题的认识论状态——是从论文提取的、是推导出来的、还是推测性的。这比 OpenIE 的简单三元组提取丰富得多。
