...
§4.3 指出 Gaia 的图结构层是概率化的 Horn 逻辑。这个观察有一个出人意料的推论：**软件包管理器（Cargo、Julia Pkg）的依赖系统在逻辑上也是 Horn clause 系统**。两者的高度相似不是巧合，而是源于相同的逻辑骨架。

### 7.1 Horn Clause 作为共同结构
...
| 系统 | Horn Clause 实例 | 语义 |
|------|-----------------|------|
| **Prolog/Datalog** | `ancestor(X,Z) :- parent(X,Y), ancestor(Y,Z)` | 逻辑推理 |
| **Cargo** | `P 可构建 :- dep_A 已满足, dep_B 已满足, dep_C 已满足` | 依赖满足 |
| **Gaia** | `结论可信 :- 前提₁ 成立, 前提₂ 成立, 前提₃ 成立` | 知识推理 |

三者的**核心数据结构完全相同**：多个输入 → 一个输出的有向超边。

### 7.2 操作语义的同构

不只是数据结构相同，连操作语义也一一对应：

| 操作 | Horn Clause | Cargo | Gaia |
|------|-------------|-------|------|
| **前向链 (forward chaining)** | 已知事实 → 推导新结论 | 所有依赖满足 → 包可构建 | 所有前提可信 → 结论可信 |
| **后向链 (backward chaining)** | 要证明 H，需证明 B₁...Bₙ | 要构建 P，需先解析其依赖 | 要验证结论，需追溯前提 |
| **拓扑排序** | 推理顺序 | 构建顺序 (build plan) | BP 传播顺序 |
| **一致性检查** | 是否可满足 | 版本是否可解析 | 信念是否收敛 |
| **增量维护** | 事实变化 → 重推受影响的结论 | 依赖更新 → 重编译受影响的包 | belief 变化 → 重传播受影响的节点 |
| **溯源/解释** | 证明树 (proof tree) | 依赖树 (`cargo tree`) | 推理链 (reasoning chain) |

### 7.3 分道扬镳：扩展方向的差异

纯 Horn clause 有一个关键性质：**单调性 (monotonicity)**——加入更多事实只会推出更多结论，永远不会推翻已有结论。
...
```
         ...
         (CDCL, PubGrub)              (Loopy BP, damping)
              |                              |
           Cargo                           Gaia
```

**Cargo 的非单调性：**
- 新增一个依赖可能导致整个依赖树不可满足（版本冲突）
- 问题从 Horn clause 推到了 **SAT/SMT** 领域
- PubGrub 算法是一个 CDCL-style 的冲突驱动求解器
- 约束是离散的、硬性的：满足或不满足，没有中间态

**Gaia 的非单调性：**
- contradiction edge 使两个命题互相抑制
- retraction edge 使旧推理链失效
- 概率是连续的，不是布尔值
- 问题从 Horn clause 推到了**概率图模型 / belief propagation** 领域
- 约束是连续的、软性的：不满足只是降低 belief，不会"编译失败"

### 7.4 为什么 Gaia 可以借鉴 Cargo

两者共享 Horn clause 骨架，这意味着 Cargo 中**纯粹基于 Horn clause 结构**的设计可以直接复用：

| 可复用（Horn clause 层面） | 不可复用（扩展方向层面） |
|---------------------------|------------------------|
| 包的命名与版本号 (semver) | 版本约束解析算法 (PubGrub) |
| spec/lock 分离 (Gaia.toml / Gaia.lock) | 全局一致性的定义（编译通过 vs belief 收敛） |
| Registry 协议（发布、下载、搜索） | "编译"的含义（独立编译 vs 全局 BP） |
| CLI 子命令结构 (`init`, `add`, `publish`) | 冲突处理（编译错误 vs belief 竞争） |
| 依赖树可视化 (`cargo tree`) | 增量更新策略（重编译 vs 增量 BP） |

判断标准很简单：**如果一个特性只依赖"多输入→单输出的有向图"（即 Horn clause 结构），就可以复用；如果它依赖"约束是离散/硬性的"，就不可以。**
...
Cargo:  T(已解析的包集合) = 已解析的包集合 ∪ 新可解析的包
Gaia:   T(当前 belief 向量) = 当前 belief 向量 ⊕ BP 消息更新
```

在纯 Horn clause（布尔、单调）下，T 保证在有限步内收敛到唯一最小不动点（Knaster-Tarski 定理）。

Cargo 加入版本约束后，可能没有不动点（不可满足的依赖），需要回溯搜索。Gaia 加入概率和矛盾后，不动点可能不唯一（Loopy BP 不保证收敛到全局最优），需要 damping 近似。

两者的扩展都是对 Horn clause 最小不动点语义的"加料"——一个加了约束满足，一个加了概率传播。
