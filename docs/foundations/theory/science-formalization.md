...
| 因子 | 连接 | 类型 | p |
|------|------|------|---|
| **W₁** | O_daily → A | **weakpoint**（induction） | < 1 |
| f_A→T₁ | A → T₁ | entailment | ≈ 1 |
| f_A→T₂ | A → T₂ | entailment | ≈ 1 |
| ⊗ | T₁ ↔ T₂ | contradiction | — |

A 是从日常观测直接归纳的规律，没有确定性前提。唯一的支撑来自 W₁（归纳跳跃），而 A 自身推导出的 T₁ ⊗ T₂ 矛盾反驳了它。
...
| 因子 | 连接 | 类型 | p |
|------|------|------|---|
| f_路径1 | H + G + C_cont + S_vacuum → V | entailment | ≈ 1 |
| **W₂** | O_media + O_air → H | **weakpoint**（abduction） | < 1 |
| **W₃** | O_media + O_air → G | **weakpoint**（induction） | < 1 |

Premise（若为假，V 从路径 1 必然不成立）：

| claim | 内容（self-contained） |
|-------|----------------------|
| **H** | 不同重量物体的下落速度差异由介质阻力造成，而非由重量本身决定 |
| **G** | 在不同密度的介质中，介质密度越低，不同重量物体的下落速度差异越小 |
| **C_cont** | 物理量随参数连续变化的趋势在极限处保持 |
| **S_vacuum** | 真空的介质密度为零（定义性） |

**路径 2 — 斜面实验直接验证：**

| 因子 | 连接 | 类型 | p |
|------|------|------|---|
| f_路径2 | V + S_plane → B_θ | entailment | ≈ 1 |
| **W₄** | E_θᵢ → V | **weakpoint**（induction） | < 1 |

Premise：**S_plane**（斜面是稀释的重力：沿面分量 = g·sin(θ)，与质量无关）。

**全局约束：** A ⊗ V [contradiction]
...
graph TD
    ...
    %% Logic path: Refute A
    A("A 重者更快") --> fA1(["ent p≈1"]) --> T1("T₁ 拖拽论证")
    ...
    %% Weakpoints as factor nodes (p < 1, to be decomposed)
    Odaily("O_daily"):::obs --> w1(["W₁<br/>induction<br/>p < 1"]):::weak --> A
    Omedia("O_media"):::obs --> w2(["W₂<br/>abduction<br/>p < 1"]):::weak --> H
    Oair("O_air"):::obs --> w2
    Omedia --> w3(["W₃<br/>induction<br/>p < 1"]):::weak --> G
    Oair --> w3
    Etheta("E_θᵢ"):::obs --> w4(["W₄<br/>induction<br/>p < 1"]):::weak --> V
...
所有 entailment 和 contradiction 因子已确定。四个 weakpoint 因子（W₁–W₄）的 p < 1 — 这正是 Step 3 要分解的目标。
...
graph TD
    ...
    %% ===== Observation path =====
    Omedia("O_media"):::obs --> w2(["W₂<br/>abduction"]):::weak --> H("H 阻力假说")
    Oair("O_air"):::obs --> w2
    Omedia --> w3(["W₃<br/>induction"]):::weak --> G("G 密度↓差异↓")
    Oair --> w3
    H --> f1(["ent p≈1"])
    G --> f1
    Ccont("C_cont 连续性") --> f1
    ...
    Sp("S_plane") --> f2
    f2 --> Btheta("B_θ 预测")
    Etheta("E_θᵢ × n"):::obs --> w4(["W₄<br/>induction"]):::weak --> V
...
2. **观测路径**：W₂ 子图提升 H，W₃ 子图提升 G → H + G + C_cont + S_vacuum 蕴含 V
3. **实验路径**：W₄ 子图中大量实验观测通过反向消息直接支撑 V
...
- A 被连球悖论压低（逻辑路径），同时 W₁ 的两个日常观测仅提供有限支撑 → A 的 belief 趋向 ε
- H 被 W₂ 的介质观测支撑 → belief 高
