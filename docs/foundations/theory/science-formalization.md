...
**G 与 O_daily 的连接：** G 预测在空气中（有阻力），阻力对轻物的相对影响更大，重物略快 — 这与 O_daily 一致。在组装图中，G 通过 entailment 连接到 O_daily，与 A 竞争解释同一观测。由于 A 导致 T₁ ⊗ T₂ 矛盾而 G 不会，BP 自动偏好 G。
...
将三个 weakpoint 的子图全部展开，汇入完整因子图。图中不再有 weakpoint factor node — 每条推理路径都由 entailment + equivalence 构成：

```mermaid
graph TD
    %% ===== W₁ expanded: A supported by daily observations (induction) =====
    A("A 重者更快"):::concl --> fAd(["ent p≈1"]) --> Bad("B_daily_A<br/>重物应更快")
    Bad --- eqd1{{"≡"}} --- Odaily("O_daily<br/>重物确实更快"):::obs
    ...
    %% ===== G explains O_daily (explaining away) =====
    G --> fGd(["ent p≈1"]) --> Bdaily("B_daily_G<br/>空气中重物略快")
    Bdaily --- eqd2{{"≡"}} --- Odaily
...
所有 weakpoint 已展开为 entailment + equivalence 子图，不再有 p < 1 的 factor node。W₁ 在此图中简化为一个预测+观测对（§3.4 展示了具体实例：石头/羽毛、铁球/木球）；W₂ 和 W₃ 完整展开。
...
**Explaining away — A 与 G 竞争解释 O_daily：**

O_daily（日常观测中重物下落更快）是 A 和 G 共享的观测节点。A 预测 B_daily_A（"速度∝重量，所以重物更快"），G 预测 B_daily_G（"空气阻力对轻物影响更大，所以重物略快"）。两条预测通过 ≡ 连接到同一个 O_daily，形成竞争。A 导致 T₁ ⊗ T₂ 矛盾，G 无矛盾且还支撑 V — BP 自动偏好 G。这不是一个单独的推理模式，而是因子图中多假说共享观测节点时 BP 的自然行为。
...
- A 被两重打击：T₁ ⊗ T₂ + A_vac ⊗ V，同时 O_daily 被 G explaining away → A 的 belief 趋向 ε
- G 被 O_media、O_air 的 abduction 支撑，又通过 explaining away 接管 O_daily → belief 高
