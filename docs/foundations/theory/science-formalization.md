...
graph TD
    ...
    G --> fGd(["ent p≈1"]) --> Bdaily("B_daily<br/>空气中重物略快")
    Bdaily --- eqd{{"≡"}} --- Odaily
...
W₂ 展开后，G 不仅通过 entailment 支撑 V，还通过 G → B_daily ≡ O_daily 解释了日常观测 — G 预测在空气中（有阻力）重物略快。O_daily 同时被 A 和 G 竞争解释：A 推出 T₁ ⊗ T₂ 矛盾，G 无矛盾且还支撑 V。BP 自动偏好 G — 这不是一个单独的推理模式，而是 BP 在多假说共享观测节点时的自然行为。
...
- A 被两重打击：T₁ ⊗ T₂（先验逻辑反驳）+ A_vac ⊗ V（后验经验反驳），同时 W₁ 的日常观测被 G explaining away → A 的 belief 趋向 ε
- G 被 W₂ 的多种介质观测支撑，又通过 explaining away 接管 O_daily → belief 高
