...
graph TD
    ...
    %% Path 2: Inclined plane
    V --> f2["ent p≈1"]
    Splane("S_plane 斜面=稀释重力") --> f2
    f2 --> Btheta("B_θ 实验预测")

    %% Logic path: Refute A
    A("A 重者更快") -->|"ent p≈1"| T1("T₁ 拖拽论证")
    A -->|"ent p≈1"| T2("T₂ 总重论证")
    T1 --- contra1{{"⊗"}}
    T2 --- contra1
    ...
    %% Context weakpoints (dashed = not yet decomposed)
    Odaily("O_daily 日常观测"):::obs -.->|"CW₁"| A
    Omedia("O_media 介质观测"):::obs -.->|"CW₂"| H
    Oair("O_air 空气观测"):::obs -.->|"CW₂"| H
    Omedia -.->|"CW₃"| G
    Oair -.->|"CW₃"| G
    Etheta("E_θᵢ 斜面实验"):::obs -.->|"CW₄"| V

    classDef obs fill:#e8f5e9,stroke:#4caf50
    classDef concl fill:#e3f2fd,stroke:#1976d2
    classDef contra fill:#ffebee,stroke:#c62828
    class A,V concl
    class contra1,contra2 contra
```

**图例**：实线 = entailment（p ≈ 1，已确定）；虚线 = context weakpoint（p < 1，待 Step 3 细化）；`⊗` = contradiction。
...
```mermaid
graph TD
    A("A 重者更快") -->|"ent p≈1"| Bs("B_stone<br/>石头应比羽毛先着地")
    A -->|"ent p≈1"| Bi("B_iron<br/>铁球应比木球先着地")
    Bps("B'_stone<br/>石头确实比羽毛先着地"):::obs ---|"≡"| Bs
    Bpi("B'_iron<br/>铁球确实比木球先着地"):::obs ---|"≡"| Bi
...
```mermaid
graph TD
    H("H 阻力假说") -->|"ent p≈1"| Bd("B_dense<br/>稠密介质中差异大")
    H -->|"ent p≈1"| Bt("B_thin<br/>稀薄介质中差异小")
    Bpd("O_media<br/>介质越稠密差异越大"):::obs ---|"≡"| Bd
    Bpt("O_air<br/>空气中差距不超过四指"):::obs ---|"≡"| Bt
...
```mermaid
graph TD
    G("G 密度↓差异↓") -->|"ent p≈1"| Bhg("B_hg<br/>水银中差异大")
    G -->|"ent p≈1"| Bw("B_water<br/>水中差异中等")
    G -->|"ent p≈1"| Ba("B_air<br/>空气中差异微小")
    Bphg("B'_hg ✓"):::obs ---|"≡"| Bhg
    Bpw("B'_water ✓"):::obs ---|"≡"| Bw
    Bpa("B'_air ✓"):::obs ---|"≡"| Ba
...
graph TD
    %% ===== Logic path (refute A) =====
    Odaily("O_daily"):::obs -->|"CW₁<br/>induction"| A("A 重者更快"):::concl
    A -->|"ent p≈1"| T1("T₁") & T2("T₂")
    T1 --- c1{{"⊗"}}:::contra --- T2

    %% ===== Observation path =====
    Omedia("O_media"):::obs -->|"CW₂ abduction"| H("H 阻力假说")
    Oair("O_air"):::obs -->|"CW₂"| H
    Omedia -->|"CW₃ induction"| G("G 密度↓差异↓")
    Oair -->|"CW₃"| G
    H --> f1["ent p≈1"]
    G --> f1
    Ccont("C_cont 连续性") --> f1
    ...
    Sp("S_plane") --> f2
    f2 --> Btheta("B_θ 预测")
    Etheta("E_θᵢ × n"):::obs -->|"CW₄ induction"| V
