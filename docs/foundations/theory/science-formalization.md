...
2. **判断与 conclusion 的依赖关系**：如果这个 claim 为假，conclusion 是否必然不成立？
   ...
   - **否** → 标记为 **context weakpoint**。推理过程中存在尚未充分分解的支撑关系，将在 Step 3 进一步细化

完成这一步后，**粗因子图**已经成形：

- Conclusion 与 premise 之间有 entailment 因子（p ≈ 1）
- Contradiction 和 equivalence 关系已经就位
- 但 context weakpoint 处的推理尚未展开 — 从原始证据到 premise 的路径上，存在 p 显著小于 1 的因子

粗因子图忠实反映了当前的形式化程度：conclusion 的 belief 已经部分由网络结构决定，但 context weakpoint 意味着部分信念仍依赖于未分解的主观判断。

### 2.3 Step 3 — 细化弱点，消除主观 p

对 Step 2 中的 context weakpoint（以及任何 p 显著小于 1 的因子），使用**子图模式**进一步分解。每种弱点对应一个可复用的子图模式，由 entailment + observation + equivalence 组合而成。分解后所有链的 p ≈ 1，不确定性从"推理链的强度"转移到"命题是否为真"。
...
```
A --[entailment, p≈1]--> B     "如果 A 为真，则预期 B"
B' (观测事实，belief 高)
B ≡ B' (equivalence)            "理论预测与观测一致"
```
...
```
G --[entailment, p≈1]--> B₁    "如果一般规律成立，则实例 1 应如此"
G --[entailment, p≈1]--> B₂    "如果一般规律成立，则实例 2 应如此"
...
G --[entailment, p≈1]--> Bₙ
...
```
G + C_cont + S_target --[entailment, p≈1]--> V
...
- 亚里士多德从 O_daily 归纳出 A
- 伽利略从 A 演绎出 T₁ 和 T₂，产生矛盾 → 反驳 A
- 伽利略从 O_media + O_air 出发，提出假说，外推到真空结论 V
- 伽利略用斜面实验 E_θᵢ 验证 V
- A 与 V 互斥

### 3.3 Step 2 — 识别弱点，构建粗因子图

#### 对 conclusion A（亚里士多德）

A 的推理过程已经包含确定性部分和弱点：

**确定性（entailment + contradiction）：**
- A --[entailment, p≈1]--> T₁、A --[entailment, p≈1]--> T₂ — 纯逻辑推导
- T₁ ⊗ T₂ [contradiction] — 同一前提推出矛盾

**弱点：** "日常观测支撑 A" 是归纳跳跃。O_daily 为假时 A 不必然不成立（A 可能有其他支撑），标记为 **context weakpoint CW₁**。
...
| 弱点 claim | 内容（self-contained） | 若为假，V 必然不成立？ | 分类 |
|-----------|----------------------|---------------------|------|
| **H** | 不同重量物体的下落速度差异由介质阻力造成，而非由重量本身决定 | 是 — 失去从介质到真空的推理基础 | **premise** |
| **G** | 在不同密度的介质中，介质密度越低，不同重量物体的下落速度差异越小 | 是 — 无法建立外推趋势 | **premise** |
| **C_cont** | 物理量随参数连续变化的趋势在极限处保持 | 是 — 无法从趋势推断极限值 | **premise** |
| **S_vacuum** | 真空的介质密度为零 | 是 — 外推缺少目标条件 | **premise**（定义性） |

Context weakpoint — 为什么相信 H 和 G？

| context weakpoint | 内容 | 说明 |
|------------------|------|------|
| **CW₂** | 介质观测 O_media, O_air 支撑假说 H | 从观测到假说是溯因推理，非蕴含 |
| **CW₃** | 介质观测 O_media, O_air 支撑模式 G | 从有限实例到一般模式是归纳推理，非蕴含 |

**路径 2 — 斜面实验直接验证：**

| 弱点 claim | 内容（self-contained） | 若为假，V 必然不成立？ | 分类 |
|-----------|----------------------|---------------------|------|
| **S_plane** | 斜面是稀释的重力：沿面分量 = g·sin(θ)，与质量无关 | 是 — 实验结果无法推广到自由落体 | **premise** |

| context weakpoint | 内容 | 说明 |
|------------------|------|------|
| **CW₄** | 有限次斜面实验支撑"对所有倾角、所有重量都成立" | 归纳推理，非蕴含 |

#### 粗因子图

```
[路径 1 — 介质→真空]
H + G + C_cont + S_vacuum --[entailment, p≈1]--> V
O_media, O_air --[?, p<1]--> H      ← CW₂，待细化
O_media, O_air --[?, p<1]--> G      ← CW₃，待细化

[路径 2 — 斜面实验]
V + S_plane --[entailment, p≈1]--> B_θ（预测）
E_θᵢ --[?, p<1]--> V                ← CW₄，待细化
...
A --[entailment, p≈1]--> T₂
T₁ ⊗ T₂ [contradiction]
O_daily --[?, p<1]--> A              ← CW₁，待细化

[约束]
A ⊗ V [contradiction]
```

Entailment 和 contradiction 已经确定。标为 `[?, p<1]` 的四条因子是 context weakpoint，其推理强度尚未展开 — 这正是 Step 3 的目标。

### 3.4 Step 3 — 细化 context weakpoint

#### CW₁：日常观测 → A（Induction 模式）

A 是"假说"，日常观测是它的"预测+观测"对：

```
A("物体下落速度与其重量成正比")
  --[entailment, p≈1]--> B_stone("如果重者更快，石头应比羽毛先着地")
  --[entailment, p≈1]--> B_iron("如果重者更快，铁球应比木球先着地")

B'_stone("日常观察：石头确实比羽毛先着地")  [observation]
B'_iron("日常观察：铁球确实比木球先着地")    [observation]

B_stone ≡ B'_stone  [equivalence]
B_iron ≡ B'_iron    [equivalence]
```

BP 效果：两个 observation 的高 belief 通过 equivalence + 反向消息提升 A。但这只有两个实例，支撑有限。

#### CW₂：介质观测 → H（Abduction 模式）

H 是假说，介质观测是它的预测+观测对：

```
H("不同重量物体的下落速度差异由介质阻力造成，而非由重量本身决定")
  --[entailment, p≈1]--> B_dense("如果阻力是原因，在稠密介质（如水银）中速度差异应该很大")
  --[entailment, p≈1]--> B_thin("如果阻力是原因，在稀薄介质（如空气）中速度差异应该很小")

B'_dense = O_media("在不同介质中比较：介质越稠密，速度差异越明显")  [observation]
B'_thin = O_air("空气中金球与铜球从 100 腕尺落下差距不超过四指")    [observation]

B_dense ≡ B'_dense  [equivalence]
B_thin ≡ B'_thin    [equivalence]
```

BP 效果：O_media 和 O_air 的高 belief 通过 equivalence + 反向消息提升 H。

如果存在竞争假说 A（"重量本身决定速度"），A 也需要预测介质观测。但 A 无法解释"为什么水银中只有金能下沉而其他金属都浮起来"和"为什么空气中差异微乎其微"——BP 的 explaining away 自动压低 A、偏好 H。

#### CW₃：介质观测 → G（Induction 模式）

G 是一般模式，各介质条件下的观测是实例：

```
G("在不同密度的介质中，介质密度越低，不同重量物体的下落速度差异越小")
  --[entailment, p≈1]--> B_hg("如果该模式成立，在水银（高密度）中差异应该很大")
  --[entailment, p≈1]--> B_water("如果该模式成立，在水（中密度）中差异应该中等")
  --[entailment, p≈1]--> B_air("如果该模式成立，在空气（低密度）中差异应该微小")

B'_hg, B'_water, B'_air  [observations]
Bᵢ ≡ B'ᵢ  [equivalence]
→ G 的 belief 被三个实例支撑
```

G 的 belief 被多个实例支撑后，路径 1 的主链完全由 entailment（p≈1）构成：

```
G + C_cont + S_vacuum --[entailment, p≈1]--> V
```

C_cont 是一个显式前提，而非隐藏在 p 值中的模糊假设。它可以被其他证据支撑（大量物理学连续性先例）或质疑（相变等不连续情况）。

#### CW₄：斜面实验 → V（Induction 模式）

V 是"假说"，每个斜面实验条件是一个预测+观测对：

```
V("在真空中，不同重量的物体以相同速率下落")
+ S_plane("斜面是稀释的重力：沿面分量 = g·sin(θ)，与质量无关")
  --[entailment, p≈1]--> B_θ₁("倾角 θ₁ 下，不同重量球的加速一致，s ∝ t²")
  --[entailment, p≈1]--> B_θ₂("倾角 θ₂ 下，不同重量球的加速一致，s ∝ t²")
  ...

B'_θ₁("实验观测：倾角 θ₁ 下确实如此") [observation]
B'_θ₂("实验观测：倾角 θ₂ 下确实如此") [observation]
...

B_θᵢ ≡ B'_θᵢ  [equivalence, 对所有测试倾角]
```
...
将四个 context weakpoint 的子图输出汇入完整的因子图：

**三条独立路径通向 V：**

1. **逻辑路径**：A → T₁ + T₂ → contradiction → A 的 belief 被压低；A ⊗ V（contradiction）→ A 低则 V 获得消极支撑
2. **观测路径**：CW₂ 子图提升 H，CW₃ 子图提升 G → H + G + C_cont + S_vacuum 蕴含 V
3. **实验路径**：CW₄ 子图中大量实验观测通过反向消息直接支撑 V

**约束关系：**

- **Contradiction**：A ⊗ V — "速度正比于重量"与"等速下落"互斥
- **Equivalence**（跨包）：V ≡ V_Newton — 牛顿从 F=ma + F=mg 推导得 a=g，与伽利略的 V 是同一结论的不同推导路径

**BP 收敛结果：**

- A 被连球悖论压低（逻辑路径），同时 CW₁ 的两个日常观测仅提供有限支撑 → A 的 belief 趋向 ε
- H 被 CW₂ 的介质观测支撑 → belief 高
...
| Step 2：识别弱点，区分 premise 与 context weakpoint | 同行评审质疑论证中的薄弱环节，判断哪些是核心假设 |
| Step 2：构建粗因子图 | 审稿人要求论证结构清晰，关键依赖关系明确 |
| Step 3：分解 context weakpoint 为 entailment + equivalence | 要求作者给出假说的可测预测，并与实际观测对照 |
