...
将 3.3 粗因子图中的三个 weakpoint 因子按 §3.4 的子图模式逐一替换，得到组装后的细因子图。此图只展示从粗图到细图的替换结果，不额外加入新的观测路径或跨包约束：

```mermaid
graph TD
    %% ===== W₁ expanded: A supported by daily observations (induction) =====
    A("A 重者更快"):::concl --> fAd1(["ent p≈1"]) --> Bs("B_stone<br/>石头应比羽毛先着地")
    A --> fAd2(["ent p≈1"]) --> Bi("B_iron<br/>铁球应比木球先着地")
    Bs --- eqd1{{"≡"}} --- Bps("B'_stone<br/>石头确实比羽毛先着地"):::obs
    Bi --- eqd2{{"≡"}} --- Bpi("B'_iron<br/>铁球确实比木球先着地"):::obs
...
粗图中的三个 weakpoint 都已被 §3.4 的子图替换，不再有 p < 1 的 factor node。W₁ 在这里按 §3.4 的两个日常实例完整展开；W₃ 仍用 θ₁、θ₂ 两个代表性实验条件示意 `E_θᵢ` 的多组实验。
...
- A 只有有限的日常实例支撑，但受到两重打击：T₁ ⊗ T₂ + A_vac ⊗ V → belief 被显著压低
- G 被 O_media、O_air 的 abduction 子图支撑 → belief 高
