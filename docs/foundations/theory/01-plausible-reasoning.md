...
**演绎（deduction）**：从一般到特殊。如果前提为真，结论必然为真。这里"必然"指数学意义上的确定性——推导步骤本身无误差（如数学证明、逻辑三段论）。不确定性仅来自前提是否成立，不来自推导过程。如果推理过程有不确定性（如计算近似、经验判断、省略了前提），应使用 `noisy_and` 建模，而非 `deduction`。
