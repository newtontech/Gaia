...
> 关于推理网络的计算结构和算法，参见 [belief-propagation.md](belief-propagation.md)。
> 本文档不定义具体的编写语言语法、Graph IR 字段布局或因子势函数公式。

**本文档是 theory/ 层的基础文件。** 下游文档在此基础上逐步引入形式结构和算法：

1. [reasoning-factor-graph.md](reasoning-factor-graph.md) — 因子图形式和逻辑算子
2. [coarse-reasoning.md](coarse-reasoning.md) — 粗推理算子
3. [belief-propagation.md](belief-propagation.md) — BP 消息传递算法
4. [science-ontology.md](science-ontology.md) — 科学知识本体论
5. [science-formalization.md](science-formalization.md) — 科学推理形式化
...
> - **因子图中的逻辑约束**（如 reasoning-factor-graph.md 中定义的硬蕴含、互斥约束）使用精确的 0/1 势函数，这完全符合概率论。
> - **数值稳定性措施**（ε-clamping 等）属于工程层面，定义在 bp/ 和实现层，不属于理论基础。
>
> 简言之：Cromwell 规则约束的是**人对世界的经验判断**，不约束因子图中的结构性逻辑约束。
...
这些概率关系在大规模科学知识上形成一个需要计算结构和算法的网络。关于推理超图（因子图）的数学结构和 Belief Propagation 算法，参见 [belief-propagation.md](belief-propagation.md)。
