...
**确定性 vs 不确定性。** 七种策略分为三类：

- **确定性策略**（p₁ = 1）：演绎、归谬、排除 — 展开后的命题网络完全由严格算子构成
- **不确定性策略**（p₁ < 1）：溯因、归纳 — 不确定性来自中间命题（预测 O、实例 Instance）的先验
- **桥梁依赖策略**（p₁ 取决于桥梁先验）：类比、外推 — 不确定性集中在桥梁主张上

**p₂ 的模式。** 值得注意的是，溯因和归纳的 p₂ = 1（观测为假时确定性地否定假说/规律），这来自严格蕴含的 modus tollens。而类比和外推的 p₂ 取决于目标结论的先验。演绎的 p₂ = π(¬C) 意味着前提不成立时结论回退到先验。
...
归谬和排除策略依赖互补关系 ⊕（定义见 `03-propositional-operators.md` §2.5）。⊕ 的理论定义是清晰的（P(A∧B)=0 且 P(¬A∧¬B)=0），但在当前 Graph IR 中尚未定义为 reasoning_type。这是已知的设计缺口 — 在 Graph IR 层新增 ⊕ 支持需要独立 PR（受保护层变更流程）。在此之前，归谬和排除策略无法完整实现。
