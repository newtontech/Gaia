...
因此，数学归纳中的 `Law` 和 `Step` 都应视为 Claim，可以合法携带 `π` 与 `belief`。真正的 future work 不是 "Template 参与 BP"，而是**量词 / 绑定变量如何在 Graph IR 中显式表示**。
...
| **量词 / 绑定变量表示** | 数学归纳 | `P(n)` 是 Template，但 `∀n.P(n)` 与 `∀n(P(n)→P(n+1))` 应作为 Claim 表示；Graph IR 尚缺少对此类 quantified claim 的显式表示 |
