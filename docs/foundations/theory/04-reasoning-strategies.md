...
**注：** 归谬需要互补关系 ⊕（定义见 `03-propositional-operators.md` §2.5）。⊕ 在当前 Gaia IR 中尚未定义 — 这是已知的设计缺口，作为 future work 处理。
...
**注：** 排除同样需要互补关系 ⊕，与归谬共享同一 Gaia IR 缺口（future work）。
...
**注：** 分情况讨论需要析取（∨）来表达穷尽性。析取可通过 De Morgan 律从 {¬, ∧} 派生：A ∨ B ≡ ¬(¬A ∧ ¬B)，因此不引入新原语。但作为独立的因子图结构，析取因子在 Gaia IR 中尚未定义为 reasoning_type — 这是已知的设计缺口，与归谬、排除的 ⊕ 缺口类似。
...
以下策略在理论层定义完整，但依赖 Gaia IR 中尚未实现的结构：

| 缺口 | 影响策略 | 说明 |
|------|---------|------|
| **⊕（互补）** | 归谬、排除 | 理论定义清晰（`03-propositional-operators.md` §2.5），Gaia IR 尚无对应 reasoning_type |
| **∨（析取因子）** | 分情况讨论 | 可通过 De Morgan 从 {¬, ∧} 派生，但作为独立因子结构尚未定义 |
| **量词 / 绑定变量表示** | 数学归纳 | `P(n)` 是 Template，但 `∀n.P(n)` 与 `∀n(P(n)→P(n+1))` 应作为 Claim 表示；Gaia IR 尚缺少对此类 quantified claim 的显式表示 |

Gaia IR 是受保护层，上述变更需要独立 PR（受保护层变更流程）。
