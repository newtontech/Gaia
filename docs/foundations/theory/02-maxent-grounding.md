...
```text
L(e_1,...,e_n | g=1) = a^s (1-a)^f
L(e_1,...,e_n | g=0) = b^s (1-b)^f
```
...
这和 [05-formalization-methodology.md](05-formalization-methodology.md) 中
...
- **本文档**：定义 posterior 的语义来源
  也就是：世界空间、约束、MaxEnt、Min-KL、乘积分解为何成立。

- **计算层**（[07-belief-propagation.md](07-belief-propagation.md) 等）：定义给定乘积分解后如何高效计算边缘后验
  也就是：具体算法、近似策略、收敛性分析。
