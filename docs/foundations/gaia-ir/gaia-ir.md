...
概率参数见 [parameterization.md](parameterization.md)。推理输出见 [../bp/belief-state.md](../bp/belief-state.md)。三者的关系见 [overview.md](overview.md)。backend-facing lowering 语义见 [lowering.md](lowering.md)。具体的概率推理算法见 [bp/](../bp/) 层。
...
所有形态折叠时均可视为单条 ↝。对 `infer` / `noisy_and` / 部分 `toolcall` / `proof`，其参数直接来自 parameterization 层；对直接 `FormalStrategy`，若需要折叠视图，则由 `FormalExpr` + 相关显式 claim prior 现算导出。
...
更完整的 backend-facing lowering 边界见 [lowering.md](lowering.md)。
...
9. helper claim 仍然是 `claim`，不引入新的 Knowledge primitive；当前 helper claim 术语只用于结构型 result claim，并额外区分 public/private 角色
