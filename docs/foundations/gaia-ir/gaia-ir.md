...
Helper claim **不是新的 Knowledge 类型**。它仍然是普通的 `claim`，只是承担了中间推理或结构支撑的角色。

当前建议区分两类：

- **语义型 helper claim**：如 `prediction`、`instance`、`bridge`、`continuity`
- **结构型 helper claim**：如 `conjunction` 结果 `M`，以及 `equivalence` / `contradiction` / `complement` / `disjunction` 的标准结果 claim

在当前 contract 下：

- helper claim 一旦进入 Gaia IR，就仍然编码为 `Knowledge(type=claim)`
- helper claim 可以是某个 `FormalStrategy` 的**私有内部节点**
- 若某个 helper claim 需要被多个 Strategy 共享或直接作为接口暴露，它就必须提升为**公共 claim**

详细约定见 [helper-claims.md](helper-claims.md)。
...
所有形态折叠时均可视为单条 ↝。对 `infer` / `noisy_and` / 部分 `toolcall` / `proof`，其参数直接来自 parameterization 层；对直接 `FormalStrategy`，若需要折叠视图，则由 `FormalExpr` + 相关语义型 helper claim prior 现算导出。
...
FormalExpr 只引用 Knowledge ID，不创建 Knowledge。展开操作需要的中间 Knowledge（如 abduction 的 prediction、deduction 的 conjunction 结果）由执行展开的 compiler/reviewer/agent 显式创建，作为独立的 `claim` 节点存在于图中；这些节点的 public/private 边界见 [helper-claims.md](helper-claims.md)。
...
溯因的不确定性在于 H 和 prediction O 的先验，不在骨架中的 Operator。需要显式中间 claim：prediction `O`；`premises` 通常包含实际观测到的 claim（这里记为 `Obs`）。`O` 默认是私有 helper claim，除非后续需要被别的 Strategy 复用。
...
归纳是多个 abduction-like 单元的并行重复。需要显式中间 claim：`Instance₁ ... Instanceₙ`。这些 `Instanceᵢ` 默认也是私有 helper claim，只有在跨 Strategy 复用时才提升为公共 claim。
...
不确定性集中在 BridgeClaim 的先验 π(BridgeClaim)，不在骨架本身。`BridgeClaim` 是公共语义型 helper claim；合取中间项 `M` 是私有结构型 helper claim。
...
与类比结构相同。不确定性在 ContinuityClaim 的先验。`ContinuityClaim` 是公共语义型 helper claim；合取中间项 `M` 是私有结构型 helper claim。
...
9. helper claim 仍然是 `claim`，不引入新的 Knowledge primitive；只额外区分 semantic/structural 与 public/private 角色
...
| helper claim 仍是 claim | 不新增 Knowledge primitive；只用 helper catalog 约束其中间角色与 public/private 边界 |
| conditional_probabilities 三层处理 | 持久化层只存外部参数；运行时 compiled 层允许每个 Strategy 拥有等效 `conditional_probabilities` 视图；直接 FormalStrategy 的该视图由 FormalExpr 与私有语义型 helper claim prior 导出 |
