...
FormalExpr 只引用 Knowledge ID，不创建 Knowledge。展开操作需要的中间 Knowledge（如 abduction 的 prediction、deduction 的 conjunction 结果）由执行展开的 compiler/reviewer/agent 显式创建，作为独立的 `claim` 节点存在于图中；这些节点的 public/private 边界见 [04-helper-claims.md](04-helper-claims.md)。
...
- 私有节点**禁止**被外部 Strategy 直接引用。原因：私有节点是 FormalStrategy 折叠（marginalization）的前提——只有严格封装在内部的节点才能被安全消去以导出等效条件概率视图
- 若确实需要被外部引用，必须先将其提升为公共 Knowledge（放入某个 Strategy 的 premises/conclusion），此时该 FormalStrategy 不再允许折叠为黑盒
...
9. FormalExpr 内部产生但不出现在任何 Strategy 的 `premises` / `conclusion` 中的中间 claim，属于该 FormalStrategy 的**私有节点**，**禁止**被外部 Strategy 直接引用。原因：私有节点是 FormalStrategy 折叠（marginalization）的前提——只有严格封装在内部的节点才能被安全消去以导出等效条件概率视图。若确实需要被外部引用，必须先将其提升为公共 Knowledge（放入某个 Strategy 的 premises/conclusion）
