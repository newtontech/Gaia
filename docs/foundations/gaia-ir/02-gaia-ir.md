...
它**不**要求 graph 只包含当前 package 自己声明的节点。
...
- 单纯把 foreign QID 写进当前 graph，**不会**铸造一个新的本地身份
...
| **`deduction`** | 无独立 strategy-level 参数 | FormalStrategy | 严格确定性推导（数学证明、逻辑三段论）。基于 directed `implication` operator，条件行为由 conjunction + directed implication skeleton 直接确定。推导步骤本身不引入不确定性——不确定性仅来自前提的 prior。如果推理有不确定性（计算误差、经验推断、省略前提），应使用 `support` |
...
2. **持久化 parameterization 输入层**
   只对需要外部概率参数的 Strategy 存 `StrategyParamRecord`，例如 `infer`。
...
实践中很少使用——大多数推理会被分类为 `support` 或命名策略。`StrategyParamRecord.conditional_probabilities: list[float]` 已支持变长列表，可存储 2^k 参数。
...
FormalStrategy(type=support, premises=[A₁,...,Aₖ], conclusion=C):
  formal_expr:
    ...
    - implication([M], conclusion=C)               # directed: M=1 → C 必须=1；warrant prior 由作者指定
```
...
对参数化叶子（`infer`），外部概率参数由 parameterization 层提供；在运行时 compiled 层，它们也对应一份直接读取的 `conditional_probabilities` 视图。
...
基于 directed `implication` operator，与 deduction 共享同一 skeleton（conjunction + directed implication），是 deduction 的 soft 版本。区别：deduction 的 implication warrant 默认 prior 接近 1（确定性推导），support 的 warrant prior 由作者指定（经验性支持）。

```
FormalStrategy(type=support, premises=[A₁,...,Aₖ], conclusion=C):
  formal_expr:
    - conjunction([A₁,...,Aₖ], conclusion=M)
    - implication([M], conclusion=C)          # directed: M=1 → C 必须=1
```

k=1 时省略 conjunction，直接 `implication([A₁], conclusion=C)`。
...
Gaia IR 管理 local package 内部的结构。公共协作生态中的 canonicalization 相关信息，来自 package、validated review reports、Official Registry issues、LKM Repo research tasks，以及后续公开的 public relation packages（见 [05-canonicalization.md](05-canonicalization.md)）。
...
IR 层的规范化边界见 [05-canonicalization.md](05-canonicalization.md)，包括：
...
| support 与 deduction 共享 skeleton | 两者结构相同（conjunction + implication），区别仅在语义强度。deduction 是 rigid（确定性推导），support 是 soft（经验性支持，prior 由作者指定） |
...
| noisy_and 废弃 | support 取代 noisy_and 作为最常用 Strategy 类型。noisy_and 是参数化 Strategy（需要外部 StrategyParamRecord），而 support 是 FormalStrategy（参数由 implication warrant prior 表达），更一致 |
