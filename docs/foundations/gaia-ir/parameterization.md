...
- **helper claim 仍按 claim 处理，但并非都需要独立 prior**：语义型 helper claim 通常需要 PriorRecord；纯结构型 helper claim 默认由 Operator 确定，不额外引入自由 prior。
- **只有参数化 Strategy 才有 conditional_probabilities**：目前包括 `infer`、`noisy_and`，以及未来若定义参数模型的 `toolcall` / `proof`。
- **直接 FormalStrategy** 不携带独立的 StrategyParamRecord：其有效条件行为由 FormalExpr、相关语义型 helper claim 的 PriorRecord，以及确定性 Operator 共同导出。
...
2. **结构推导层**：直接 FormalStrategy 的行为由 `FormalExpr` + 语义型 helper claim prior 决定；纯结构型 helper claim 由 Operator 确定
...
- **显式 claim 先验**：语义型 helper claim 与其他不确定 claim 的 PriorRecord——直接 FormalStrategy 的有效条件行为由这些 prior 与内部 skeleton 导出。
...
- 对直接 FormalStrategy：对其**私有内部、且承载不确定性的 helper claim** 做 marginalization，从 `FormalExpr` + PriorRecord 导出
...
- 每个直接 FormalStrategy 所依赖的语义型 helper claim 都必须有 PriorRecord

纯结构型 helper claim 若只是 Operator 的确定性结果，则不要求独立 PriorRecord。
...
| 直接 FormalStrategy（`deduction` 至 `case_analysis`） | 不单独赋持久化 strategy 参数；其有效条件行为由 FormalExpr + 语义型 helper claim 的 PriorRecord 导出。纯结构型 helper claim 作为 Operator 结果，不默认引入独立 prior |
