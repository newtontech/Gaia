...
>
> **⚠️ Protected Contract Layer** — 本目录定义 CLI↔Official Registry 的结构边界。变更需要独立 PR 并经负责人审查批准。

## 1. Scope 划分

Gaia IR 只管理 **local package 内部的结构表示**。  
跨 package 的“是不是同一个命题”“是不是旧论证的改写”“会不会 double count”属于 **Official Registry 公开维护的判断结果**，不是 core IR 里的 runtime primitive。
...
| `equivalence` Operator | IR | 作者在包内显式声明两个 claim 的真值等价 |
| `metadata` / provenance | IR | 保留来源、上下文、相关引用等可审计信息 |
| accepted package versions | Official Registry | 哪些 package 版本进入公共记录 |
| accepted cross-package judgments | Official Registry | 对 duplicate / refinement / contradiction / unresolved 等判断的公开记录 |
| registry indexes | Official Registry | 为任意下游 LKM 暴露可查询的 occurrence 与 judgment 信息 |

**边界原则：**

- Gaia IR 负责把 package 内发生过什么写清楚
- Official Registry 负责把哪些 package / judgment 被接受写清楚
...
这让 registry 能无歧义地引用 “package A 的 claim X” 和 “package B 的 claim Y”，而不必先假定二者已经是同一个命题。
...
- 作为更昂贵语义比对前的快速预筛选
- 辅助 registry 建立 duplicate / refinement 审查队列
...
这些信息本身不直接做概率运算，但它们是 registry 和 reviewer 判断是否存在 double counting 风险的重要上下文。
...
这些都是 **public review / registry judgment** 问题，而不是 package-local runtime operator 问题。
...
这类判断本身应通过新的 Gaia package（例如 investigation / review / curation artifact）进入 GitHub 和 Official Registry 的公开记录。

core IR 在这里的职责不是直接“合并图”，而是保证这些 judgment package 至少能够：

- 无歧义地引用既有 occurrence
- 提供可审查的论证内容
- 保留产生该 judgment 的 provenance

一旦这类 package 被接受，Official Registry 至少应公开维护：

- 被接受的 package 版本
- occurrence 引用（package/version/qid）
- 被接受的 cross-package judgment 及其目标对象
- supersession / refinement / rebuttal 等公开关系
