...
| **deduction / reductio / elimination / mathematical_induction / case_analysis** | `[p]`（折叠时） | FormalStrategy | fully expanded 时通常直接写成确定性 Operator skeleton |
| **abduction / induction / analogy / extrapolation** | `[p]`（折叠时） | FormalStrategy | 一旦识别出命名结构即可直接 formalize；若更大论证要保留 hierarchy，则由外层 CompositeStrategy 组合这些 FormalStrategy |
...
FormalStrategy 的确定性展开结构——由 Operator 列表构成。中间 Knowledge 不由 FormalExpr 自动创建，而需显式存在于图中。不是独立实体，而是 FormalStrategy 的嵌入字段。`deduction`/`reductio`/`elimination`/`mathematical_induction`/`case_analysis` 以及 `abduction`/`induction`/`analogy`/`extrapolation` 都可以直接表现为 FormalStrategy；如果需要保留更大的层次结构，则由外层 CompositeStrategy 组合这些子结构。`toolcall`/`proof` 暂无标准 FormalExpr 模板。Schema 见 [gaia-ir.md](gaia-ir.md) §4。
...
- **strategy_params**：所有推理类 Strategy 都有 conditional_probabilities（`infer`=2^k 参数 CPT，`noisy_and`=单参数 `[p]`，其余命名策略在折叠时通常也用单参数 `[p]`）。
