...
其中 **helper claim 仍然是 `claim`**，不是新的 Knowledge 类型；当前术语主要指结构型 result claim。它可以是 public helper claim，也可以是某个 `FormalStrategy` 的 private helper claim。详细约定见 [04-helper-claims.md](04-helper-claims.md)。
...
详细 schema 见 [02-gaia-ir.md](02-gaia-ir.md) §2。

### Operator（结构约束）

确定性逻辑关系（equivalence, contradiction, complement, implication, disjunction, conjunction）。对应 theory Layer 3 的势函数，所有算子均确定性（ψ ∈ {0, 1}，无自由参数）。当前 contract 下，每个 Operator 都有 `conclusion`；对关系型 Operator，这个 `conclusion` 是结构型 helper claim。Schema 见 [02-gaia-ir.md](02-gaia-ir.md) §3 与 [04-helper-claims.md](04-helper-claims.md)。

### FormalExpr（data class，非顶层实体）

FormalStrategy 的确定性展开结构——由 Operator 列表构成。中间 Knowledge 不由 FormalExpr 自动创建，而需显式存在于图中；这些中间 claim 统一按 helper claim 规则管理。不是独立实体，而是 FormalStrategy 的嵌入字段。`deduction`/`reductio`/`elimination`/`mathematical_induction`/`case_analysis` 以及 `abduction`/`induction`/`analogy`/`extrapolation` 都可以直接表现为 FormalStrategy；如果需要保留更大的层次结构，则由外层 CompositeStrategy 组合这些子结构。`toolcall`/`proof` 暂无标准 FormalExpr 模板。Schema 见 [02-gaia-ir.md](02-gaia-ir.md) §4 与 [04-helper-claims.md](04-helper-claims.md)。
...
其中两个 ID 命名空间如下（global 层不存储 steps，Knowledge 内容通常经 `representative_lcn` 间接引用）：
...
`Knowledge.content_hash` 独立于 `id`：相同内容的 local 节点可以有不同 `lcn_id`，但共享同一个 `content_hash`。规范化（lcn → gcn 映射）见 [05-canonicalization.md](05-canonicalization.md)，完整身份规则见 [03-identity-and-hashing.md](03-identity-and-hashing.md)。

### 图哈希

LocalCanonicalGraph 有确定性哈希 `ir_hash = SHA-256(canonical JSON)`，用于编译完整性校验——审查引擎重新编译并验证匹配。GlobalCanonicalGraph 是增量变化的，不使用整体哈希。`ir_hash` 不等于任何 Knowledge 的 `id` 或 `content_hash`。
