...
二者严格分离。Gaia IR 有 local 和 global 两层；Gaia IR 核心 contract 中的 Parameterization 只作用在 GlobalCanonicalGraph 上。若某些 local-only backend 需要临时参数输入，那属于 backend-private workflow，不属于本目录的持久化参数契约。后端（如 BP）同时消费 IR 和 Parameterization 来产出最终结果。
...
- **GlobalCanonicalGraph**：LKM 服务器上的全局结构索引，由多个 local 子图通过 canonicalization 合并而成

每个 local 子图是独立编译、独立校验的单元。Global 图则是所有已发布子图的跨包整合。

### 子图合并与封装边界

当不同 local 子图合并到 global 图时，canonicalization 会识别跨包的语义等价节点并映射到同一个 `gcn_id`。但 FormalStrategy 内部的 **private claims 必须保持封装**——它们不应与其他子图中的 claim 合并或被外部引用。
