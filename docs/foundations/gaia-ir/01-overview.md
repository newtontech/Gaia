...
- **Parameterization** 是 canonical graph 上的概率参数层——review 过程为每个 Knowledge 和 Strategy 赋予可信度。IR 代码当前在 `LocalCanonicalGraph` 上实现参数化；全局层参数化（在 `GlobalCanonicalGraph` 上）由 LKM 负责。

二者严格分离。Gaia IR 核心 contract 定义参数化模型，该模型可用于 local 和 global 两层。IR 代码实现 local 层；global 层的持久化参数契约由 LKM 管理。后端（如 BP）同时消费 IR 和 Parameterization 来产出最终结果。
...
- **LocalCanonicalGraph**：来自单个 package 的编译产物，包含该 package 内的 Knowledge、Strategy、Operator。**IR 代码直接管理此层。**
- **GlobalCanonicalGraph**（LKM 层）：LKM 服务器上的全局结构索引，由多个 local 子图通过 canonicalization 合并而成。**由 LKM 负责构建和维护，IR 代码不直接操作。**

每个 local 子图是独立编译、独立校验的单元。Global 图则是所有已发布子图的跨包整合（由 LKM 负责）。

### 子图合并与封装边界

当不同 local 子图合并到 global 图时（由 LKM 负责），canonicalization 会识别跨包的语义等价节点并映射到同一个 `gcn_id`。但 FormalStrategy 内部的 **private claims 必须保持封装**——它们不应与其他子图中的 claim 合并或被外部引用。

原因：private claim 的存在意义是服务于所属 FormalStrategy 的内部结构。如果合并过程把两个不同 FormalStrategy 的内部中间节点当作"语义等价"而合并，就破坏了各自的封装边界，导致折叠不再安全。

因此，canonicalization 对 FormalExpr 内部的 private claim 应保持其 FormalStrategy 归属不变，不做跨 FormalStrategy 合并。
...
**Global 层示例**（跨包，展示三种 Strategy 形态；由 LKM 构建和维护，IR 代码不直接操作）：
      ...
      "formal_expr": {
        "operators": [
          {"operator_id": "gco_1...", "operator": "conjunction",
           "variables": ["gcn_a1...", "gcn_b2..."], "conclusion": "gcn_m1..."},
          {"operator_id": "gco_2...", "operator": "implication",
           "variables": ["gcn_m1..."], "conclusion": "gcn_d4..."}
        ]
      }
    }
  ],
...
Global 层 Knowledge 通常不存储 content（通过 `representative_lcn` 引用 local 层）。LKM 服务器直接创建的 Knowledge（包括 FormalExpr 中间 Knowledge 如 `gcn_m1`）无 local 来源，content 直接存在 global 层。`representative_lcn` 和 `local_members` 是 global 层（LKM）专有字段，IR 代码中不使用。
...
- **对象身份**：Knowledge 使用 QID（local）/ `gcn_`（global，LKM 层）；Strategy 使用 `lcs_`/`gcs_`；Operator 使用 `lco_`/`gco_`
- **内容指纹**：`Knowledge.content_hash`
- **图完整性哈希**：`LocalCanonicalGraph.ir_hash`

其中两个 ID 命名空间如下（global 层不存储 steps，Knowledge 内容通常经 `representative_lcn`（LKM 层）间接引用）：
...
| **GlobalCanonicalGraph**（LKM 层） | 跨包 | `gcn_` | `gcs_`, `gco_` | 引用 representative，Strategy 无 steps（结构索引）+ Operator |
...
LocalCanonicalGraph 有确定性哈希 `ir_hash = SHA-256(canonical JSON)`，用于编译完整性校验——审查引擎重新编译并验证匹配。GlobalCanonicalGraph（LKM 层）是增量变化的，不使用整体哈希。`ir_hash` 不等于任何 Knowledge 的 `id` 或 `content_hash`。

结构校验的分层视图见 [08-validation.md](08-validation.md)。

## 二、Parameterization — 参数

Parameterization 是 canonical graph 上的概率参数层。它由**原子记录**构成，不同 review 来源（不同模型、不同策略）产出不同的记录。IR 代码中字段名为 `knowledge_id`（通用），global 层使用 `gcn_` ID（由 LKM 负责）。

### 存储层

```json
// PriorRecord（每条一个 Knowledge）
// 注：IR 代码中字段名为 knowledge_id；global 层使用 gcn_ ID（LKM 层）
{"knowledge_id": "gcn_8b1c...", "value": 0.7, "source_id": "src_001", "created_at": "..."}
{"knowledge_id": "gcn_8b1c...", "value": 0.8, "source_id": "src_002", "created_at": "..."}
...
| **GlobalCanonicalGraph**（LKM 层） | 跨包 Knowledge（引用 local QID）+ 全局 Strategy（无 steps，FormalStrategy 含 FormalExpr）+ Operator | 每次 ingest/curation 更新 |
| **CanonicalBinding**（LKM 层） | QID → gcn 映射记录 | 每次 ingest 更新 |
| **PriorRecord** | claim 的 prior（每条记录携带 source；IR 代码字段名 `knowledge_id`） | 每次 review 追加 |
...
- `gaia/gaia_ir/models.py` -- `LocalCanonicalGraph`, `Knowledge`, `Strategy`
- `gaia/gaia_ir/operator.py` -- `Operator`, `OperatorType`
- `gaia/gaia_ir/strategy.py` -- `CompositeStrategy`, `FormalStrategy`, `FormalExpr`
- `gaia/gaia_ir/parameterization.py` -- `PriorRecord`, `StrategyParamRecord`, `ParameterizationSource`
- `libs/storage/models.py` -- global `Knowledge`, `BeliefSnapshot`
- *LKM 层（尚未实现）：* `CanonicalBinding`、`GlobalCanonicalGraph` 构建与 canonicalization
