...
3. **Belief Propagation**——在 Graph IR 上的概率推理。根据先验和因子势函数计算所有知识节点的后验信念。参见 `../bp/inference.md`。

## 两个产品表面

Graph IR 是 Gaia 两个产品表面之间的边界：

| | CLI | LKM（服务器） |
|---|---|---|
| **范围** | 本地，单个包 | 全局，多包 |
| **输入** | Gaia Lang 源文件（.typ 文件） | 已发布的 Graph IR |
| **编译** | Gaia Lang -> Graph IR | 不适用——接收 Graph IR |
| **推理** | 在本地规范图上运行本地 BP | 在全局规范图上运行全局 BP |
| **存储** | LanceDB + Kuzu（嵌入式） | LanceDB + Neo4j + 向量 |
| **额外服务** | — | 审查、策展、全局规范化 |

CLI 是 Graph IR 的**前端**——类比于 Clang 是 LLVM IR 的前端。LKM 永远不会看到 Gaia Lang；它纯粹在 Graph IR 上操作。
...
- **前端独立性**——未来的前端可以在不使用 Typst 的情况下生成 Graph IR
- **CLI 与 LKM 解耦**——LKM 在 Graph IR 上进行验证和操作，独立于编写表面
- **结构与参数分离**——Graph IR 仅携带结构；概率存在于参数化覆盖层中
...
| Graph IR | LLVM IR / MIR / SIL | 共享中间表示 |
