...
**为什么用 embedding 而非精确匹配：** "YBCO 在 92K 以下超导" 和 "YBa₂Cu₃O₇ 的 Tc 为 92 ± 1 K" 内容不同但语义相同。精确匹配（content hash）会漏掉这类情况。漏掉的语义重复由 LKM 的 curation 流程补充发现——LKM 在全局推理过程中识别 equivalence 候选，确认后以 curation 包的形式提交合并（详见 [07-review-and-curation.md](07-review-and-curation.md) 的 LKM Curation 部分）。
