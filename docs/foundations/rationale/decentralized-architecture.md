...
| 参与者 | 交互方式 |
|--------|---------|
| 作者 | git push 到自己的包仓库；浏览 LKM Repo 的 research tasks 寻找研究机会；向 Official Registry 请求注册 |
...
```mermaid
%%{init: {'theme': 'base', 'flowchart': {'htmlLabels': true, 'nodeSpacing': 56, 'rankSpacing': 76}} }%%
flowchart LR
    subgraph Actors[" "]
        direction TB
        Author["作者<br/>人类 / AI agent"]:::author
        RS["Review Server<br/>独立部署 × N"]:::review
        LKM["LKM Server<br/>全局推理 / curation"]:::lkm
    end

    subgraph GIT["Git Server<br/>GitHub / GitLab / Gitea"]
        direction TB
        subgraph GitTop[" "]
            direction LR
            PKG["Knowledge Repo<br/>知识包仓库"]:::pkg
            REG["Official Registry<br/>注册中心"]:::registry
        end
        LKMR["LKM Repo<br/>research tasks"]:::lkmrepo
    end

    Author -- "① 创建知识包" --> PKG
    Author -- "② 请求审核" --> RS
    RS -- "③ 写回 review report" --> PKG
    PKG -- "④ 带 review 注册" --> REG

    Author -. "浏览 research tasks" .-> LKMR
    LKM -- "⑤ 发布 research task" --> LKMR
    LKM -- "⑥ 创建 curation 包" --> PKG
    LKM -- "⑦ 请求 curation 审核" --> RS
    LKM -- "⑧ 注册 curation / belief" --> REG

    REG -. "拉取全局图数据" .-> LKM
    REG -. "同步最新可信度" .-> PKG

    classDef author fill:#DBEAFE,fill-opacity:0.80,stroke:#2563EB,stroke-width:2px,color:#0F172A;
    classDef review fill:#FEF3C7,fill-opacity:0.82,stroke:#D97706,stroke-width:2px,color:#0F172A;
    classDef lkm fill:#CCFBF1,fill-opacity:0.82,stroke:#0F766E,stroke-width:2px,color:#0F172A;
    classDef pkg fill:#E0F2FE,fill-opacity:0.76,stroke:#0284C7,stroke-width:2px,color:#0F172A;
    classDef registry fill:#DCFCE7,fill-opacity:0.78,stroke:#16A34A,stroke-width:2px,color:#0F172A;
    classDef lkmrepo fill:#FFEDD5,fill-opacity:0.78,stroke:#EA580C,stroke-width:2px,color:#0F172A;

    style Actors fill:transparent,stroke:transparent
    style GitTop fill:transparent,stroke:transparent
    style GIT fill:#F8FAFC,fill-opacity:0.54,stroke:#94A3B8,stroke-width:2px,color:#334155

    linkStyle default stroke:#64748B,stroke-width:2px,color:#0F172A
    linkStyle 0,3 stroke:#2563EB,stroke-width:2.5px,color:#1E3A8A
    linkStyle 1,2,7 stroke:#D97706,stroke-width:2.5px,color:#92400E
    linkStyle 5,6,8 stroke:#0F766E,stroke-width:2.5px,color:#134E4A
    linkStyle 4,9,10 stroke:#94A3B8,stroke-width:2px,color:#475569,stroke-dasharray: 6 4
```
...
| 步骤 | 描述 | 详见 |
|------|------|------|
| ① 创建包 | 作者创建知识包到 Package Repo | [authoring-and-publishing.md](authoring-and-publishing.md) |
...
| ④ 注册 | 作者带着 review report 向 Official Registry 请求注册 | [registry-operations.md](registry-operations.md) |

**LKM 流：**

| 步骤 | 描述 | 详见 |
|------|------|------|
| ⑤ 发布 research task | LKM 在 LKM Repo 创建 Issue，发布候选发现 | [review-and-curation.md](review-and-curation.md) |
| ⑥ 创建 curation 包 | 候选确认后，LKM 创建 curation 包到 Package Repo | [review-and-curation.md](review-and-curation.md) |
| ⑦ curation 包审核 | 同作者流——经 Review Server 审核 | [review-and-curation.md](review-and-curation.md) |
| ⑧ 回写可信度 | LKM 全局推理结果回写到 Registry | [belief-flow-and-quality.md](belief-flow-and-quality.md) |
