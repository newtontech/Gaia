...
graph TB
    ...
    subgraph row1[" "]
        direction LR
        Author["👤 <b>作者</b><br/>人类 / AI agent<br/><i>从研究中创建知识包</i>"]
        LKM["🖥 <b>LKM Server</b><br/>全局推理引擎<br/><i>全局 BP + 发现跨包关系<br/>→ 创建 curation 包</i>"]
    end

    %% ═══════════════════════════════════════════════════
    %% 第 2 层：Review Server（为两者服务）
    %% ═══════════════════════════════════════════════════
    subgraph row2[" "]
        direction LR
        RS["⚙ <b>Review Server ×N</b>（独立部署）<br/>LLM/agent 审核员<br/><i>审核推理逻辑 · 给条件概率</i>"]
    end

    %% ═══════════════════════════════════════════════════
    %% 第 3 层：两个 Git Repo（并列）
    %% ═══════════════════════════════════════════════════
    subgraph row3[" "]
        direction LR
...
**布局说明：**
- **第 1 层（顶部）：** 两类并列贡献者——人类/Agent 和 LKM Server
- **第 2 层（中间）：** Review Server，为两者服务（审核人类的知识包和 LKM 的 curation 包）
- **第 3 层（底部）：** 两个 git 仓库——Package Repo 和 Official Registry
- 实线 = 数据流，虚线 = 拉取/辅助
