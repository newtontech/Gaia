...
本地推理预览满意后，作者向一个或多个 Review Server 请求审核。Review Server 会把 `review report` 作为 PR 提交到作者自己的仓库；作者合并足够的 report 后，再向 Official Registry 请求注册。

### 为什么需要审核

作者的 `gaia infer` 只能给出可信度预览。进入官方流程后，Review Server（LLM/agent）会给出两类参数：新命题的初始 prior，以及推理链的条件概率。它审核的是包内推理过程和新命题的证据质量，不直接裁决跨包结构关系。最终进入官方流程的，是那些已经被合并进包内 `review report` 文件夹、并在 Registry 入库时通过校验的报告。
...
包内 `.gaia/reviews/` 达到 Registry 要求的 minimal review set
  ↓
向 Official Registry 请求注册
  ↓
Registry 校验 review reports 的存在性、来源、数量和格式
```
...
发布流程：
  ...
  4. 请求一个或多个 Review Server 审核，并把 review report PR 合并进 `.gaia/reviews/`
  5. review reports 达到 Registry 的 minimal policy 后，请求 Official Registry 注册（见 04-registry-operations.md）
```
