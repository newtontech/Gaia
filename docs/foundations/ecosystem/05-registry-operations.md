...
本文档描述 Official Registry（官方注册仓库）的业务逻辑：包如何注册、命题如何去重、新证据如何进入待审队列。
...
包合并到 Official Registry 后，进入去重流程。
...
→ 直接绑定到 Official Registry 中已有的同一命题。不创建新实体，不影响已有可信度。
...
**为什么用 embedding 而非精确匹配：** "YBCO 在 92K 以下超导" 和 "YBa₂Cu₃O₇ 的 Tc 为 92 ± 1 K" 内容不同但语义相同。精确匹配（content hash）会漏掉这类情况。漏掉的语义重复由 LKM 的 curation 流程补充发现——LKM 在全局推理过程中识别 equivalence 候选，确认后以 curation 包的形式提交合并（详见 [06-review-and-curation.md](06-review-and-curation.md) 的 LKM Curation 部分）。

### 去重完成后的状态

去重完成后，包中的所有命题都有了在 Official Registry 中的全局身份。但包中的**推理链尚未生效**——它们进入待审队列。
...
作者后续补充 review：
  ...
  → 通过 PR 将 review report 补充到 Official Registry
  → CI 验证 → 推理链激活
```
...
- [03-decentralized-architecture.md](03-decentralized-architecture.md) — 架构总纲，Registry 在分层中的定位
- [04-authoring-and-publishing.md](04-authoring-and-publishing.md) — 作者从创建包到发布的完整旅程（注册的前置流程）
- [06-review-and-curation.md](06-review-and-curation.md) — Review Server 审核 + LKM curation（去重的补充发现机制）
- [07-belief-flow-and-quality.md](07-belief-flow-and-quality.md) — 增量推理、错误修正场景
