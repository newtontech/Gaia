...
作者的工作通常建立在已有知识之上。依赖的指向方式取决于对方是否已在 Official Registry 注册（参见 [03-decentralized-architecture.md](03-decentralized-architecture.md) 的架构分层）：
...
**已注册包优先引用 Registry 标识**，因为 Registry 提供全局身份、去重和可信度数据。**未注册包通过 git URL + tag 引用**——两个人互相引用对方的 GitHub 仓库就能建立知识关系，不需要任何中心服务。注册中心是可选的增值层，不是前提条件。
...
发布流程：
  ...
  3.（可选）向 Official Registry 请求注册（见 05-registry-operations.md）
```
...
- **提交新发现：** 如果在研究过程中发现其他包之间可能存在的关系，也可以在对应的 LKM Repo 提交 issue
...
这些局限由 Level 1（Official Registry）和 Level 2（LKM Server）解决。见 [05-registry-operations.md](05-registry-operations.md)。
