...
Setting 不携带 prior，不参与 BP。可作为 factor 的承载性前提（structural dependency），但不创建 BP 边。对齐 [reasoning-hypergraph.md §6.2](../theory/reasoning-hypergraph.md) 和 [graph-ir.md §1.2](../graph-ir/graph-ir.md)。

Claim 可以通过 `from:` 引用 setting 作为前提。Setting 出现在编译后 FactorNode 的 `premises` 列表中，但按 graph-ir §2.5 的 BP 参与规则，non-claim premise 不参与 BP 消息传递。
...
Equivalence 同理（`reasoning_type: equivalent`）。参见 `graph-ir/graph-ir.md` 中 FactorNode schema 的定义。
...
**`kind:`** —— `#claim` 上的可选科学子类型（如 `"observation"`、`"hypothesis"`、`"computation"`、`"python"`）。记录证据类型；不改变 Graph IR 拓扑结构。`kind` 编译为 `KnowledgeNode.metadata: {schema: <kind>}`。
...
论证策略（`#abduction` 等）生成细因子图，由 entailment + equivalence + contradiction 组合而成，不涉及 noisy-AND——推理效果通过 BP 消息传递协作实现。

参见 [reasoning-hypergraph.md §4.1](../theory/reasoning-hypergraph.md) 和 [science-formalization.md §2.4](../theory/science-formalization.md)。

## 论证策略

四个论证策略（argumentation strategies）提供比 `from:` 更精确的推理结构。编译器自动从粗因子图展开为细因子图。
