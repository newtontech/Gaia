...
已提交 package 中的 authored knowledge 对象。用于 provenance、检索、authoring-layer 拓扑和 review 历史保留；它不是全局 BP 的变量节点。全局 BP 的变量节点由 `GlobalCanonicalNode` 承担。
...
**相对 v1.1 的变化：** key 从 `(knowledge_id, version)` 收口为 `global_canonical_id`。BP 在 Global Canonical Graph 上运行。
...
| `modules` | Module 全部字段（含 imports） | 按 package_id 查询 | **imports 改为 package-scoped target + dependency** |
| `knowledge` | Knowledge 全部字段 + embedding | BM25 搜索、向量搜索、按 (id, version) 精确查 | **+kind, +parameters；prior 语义更新为 reviewed prior** |
...
| `belief_history` | BeliefSnapshot | 按 global_canonical_id 查询演化 | **key 变为 global_canonical_id** |
...
`prior` 列保留，但语义更新为该 authored knowledge unit 的 reviewed prior history。全局 BP 实际使用的 prior 读取自 `GlobalInferenceState.node_priors`。
...
GraphStore 存储两层图拓扑：Authoring layer（Knowledge / Chain）和 Global inference layer（GlobalCanonicalNode / Factor）。Package-local raw/local canonical graph 作为 submission artifact 保存在 ContentStore 中；全局 BP-facing 拓扑则以 `GlobalCanonicalNode` 为变量节点。
...
关系 — Global inference layer（Factor，v2.0 新增）:
...
Chain 是 authoring-layer 概念（多步推理叙事），Factor 是 Graph IR / BP 概念（编译后因子，BP 直接消费）。粒度不同：Chain 按步有多个 PREMISE/CONCLUSION 关系，Factor 是整条 chain 编译为一个 reasoning factor，只有 premises→conclusion。全局 BP 变量节点是 `GlobalCanonicalNode`，不是 `Knowledge`；`Knowledge` 保留给 provenance、authoring-layer 浏览和搜索。通过 `FactorNode.source_ref` 可以从全局 factor 回溯到对应的 package/module/knowledge 来源。

**CanonicalBinding 的位置：** `CanonicalBinding` 是 review/registry-side record，作为 ContentStore 的 first-class record 存储。它不是全局 BP 拓扑本身；如果某个后端为了查询便利物化了 lookup edge，也应视为 derived index，而不是 BP source of truth。
...
get_subgraph(node_id, layer, max_nodes) → Subgraph
    → layer = "authoring" 时 node_id 为 knowledge_id
    → layer = "global_inference" 时 node_id 为 global_canonical_id
    → 返回相关 Knowledge / GlobalCanonicalNode + Chain / Factor 节点及关系

get_neighbors(node_id, layer, direction, max_hops) → Subgraph
    → layer = "authoring" 时 node_id 为 knowledge_id
    → layer = "global_inference" 时 node_id 为 global_canonical_id
```
...
class ContentStore(ABC):
    ...
    # 写入 — package Graph IR artifact (v2.0 新增)
    ...
    # 读取 — package Graph IR artifact (v2.0 新增)
    async def list_factors(self) -> list[FactorNode]
    async def get_factors_by_package(self, package_id: str) -> list[FactorNode]
...
class GraphStore(ABC):
    ...
    # 写入 — Global inference layer (v2.0 新增)
...
- 新增 `write_factor_topology`（GlobalCanonicalNode ↔ Factor 的 BP-facing 拓扑写入）
- 新增 `write_global_topology`（根据 bindings 生成全局 inference 拓扑；`CanonicalBinding` 记录本身仍以 ContentStore 为 source of truth）
...
integrate(bindings, global_nodes, inference_state):
    ...
    3. GraphStore.write_global_topology(bindings, global_nodes)
       ← 以 bindings 作为 remapping 输入，写 GlobalCanonicalNode ↔ Factor 拓扑
