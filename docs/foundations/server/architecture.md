...
Server 架构采用 **Write Side / Read Side 分离**，类似推荐系统中的读写分离模式：

### Write Side（数据入库路径）

负责 package 从提交到入库的全流程，以及全局图的离线维护：
...
| 服务 | 说明 |
|------|------|
| **Query Service** | 面向 agent 的知识搜索和子图探索 |
...
三层分层架构：Transport → Domain Services → Storage，Domain Services 按 Write/Read Side 组织。
...
│  │                                                             │  │
│  │  ┌─────────────────────────────────────────────────────┐   │  │
│  │  │ Review Service (IngestionService)                    │   │  │
│  │  │                                                      │   │  │
│  │  │ submit(pkg) → 1. validate (re-compile + graph diff)  │   │  │
│  │  │             → 2. canonicalization audit               │   │  │
│  │  │             → 3. multiple agent review                │   │  │
│  │  │             → 4. gatekeeper (global match + integrate)│   │  │
│  │  │                                                      │   │  │
│  │  │ pkg + report → global IR → global inference          │   │  │
│  │  └─────────────────────────────────────────────────────┘   │  │
│  │                                                             │  │
│  │  ┌──────────────────┐  ┌────────────────────┐              │  │
│  │  │ BP Service        │  │ Curation Service    │              │  │
│  │  │                   │  │                     │              │  │
│  │  │ run_global()      │  │ cluster_similar()   │              │  │
│  │  │ run_subgraph()    │  │ discover_conflicts()│              │  │
│  │  │ schedule_update() │  │ inspect_structure() │              │  │
│  │  │                   │  │ cleanup()           │              │  │
│  │  └──────────────────┘  └────────────────────┘              │  │
│  │       ↑ 离线定期：Curation 先做结构维护，BP 再跑推理更新 ↑    │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                                                   │
│  ┌─ Read Side ─────────────────────────────────────────────────┐  │
│  │                                                             │  │
│  │  ┌──────────────┐                                          │  │
│  │  │ QueryService │  MVP: agents → research                  │  │
│  │  │              │                                          │  │
│  │  │ search()     │  一等消费者是 AI agents                   │  │
│  │  │ read()       │  核心用例是科研探索                        │  │
│  │  │ subgraph()   │                                          │  │
│  │  └──────────────┘                                          │  │
│  └─────────────────────────────────────────────────────────────┘  │
...
agent: self-review  → candidate weak points + priors (不提交)
...
| **GraphStore (Kuzu / Neo4j)** | 图拓扑：Authoring layer 的 Knowledge → Chain → Knowledge，以及全局推理层的 GlobalCanonicalNode → Factor → GlobalCanonicalNode | 邻居遍历、子图提取、topology search |
...
Package、Chain、ChainStep、KnowledgeRef、ProbabilityRecord、Resource、ResourceAttachment — schema 不变，语义不变。Chain 仍是 authoring-layer 概念。Module 保持总体结构，但 `imports` 语义已更新为 package-scoped target + `dependency: direct/indirect`。BeliefSnapshot 仍保留，但其 key 语义已切换到 `global_canonical_id`。

#### 扩展的实体

**Knowledge** — authored knowledge / provenance object。它保留 package-level 内容、reviewed prior 和检索信息，但不是全局 BP 的变量节点：
...
Knowledge:
    ...
    prior:           float            # reviewed prior attached to this authored knowledge unit; historical/provenance reference only
...
**FactorNode** — Graph IR 的持久化因子。进入全局 BP 时，`premises/contexts/conclusion` 都引用 `global_canonical_id`，而不是 `knowledge_id`：
...
**CanonicalBinding** — 本地→全局身份映射（review/registry-side 记录）。它是 registry metadata，不是全局 BP 拓扑节点：
...
| `belief_history` | BeliefSnapshot（key = global_canonical_id） | key 语义更新 |
...
关系 — Global inference layer（Factor）:
    (:GlobalCanonicalNode)-[:FACTOR_PREMISE]->(:Factor)    # 直接依赖，创建 BP 边
    (:GlobalCanonicalNode)-[:FACTOR_CONTEXT]->(:Factor)    # 间接依赖，不创建 BP 边
    (:Factor)-[:FACTOR_CONCLUSION]->(:GlobalCanonicalNode) # conclusion
...
Chain 是 authoring-layer 概念（多步推理，叙事结构）。FactorNode 是 Graph IR / BP 概念（编译后的因子，BP 直接消费）。保留 Chain 层用于 package 结构浏览和搜索。Factor 层用于 BP 和推理审计，但其变量节点是 `GlobalCanonicalNode`，不是 `Knowledge`。两层通过 FactorNode.source_ref 关联——source_ref 指向生成该 factor 的 Chain。
...
### 5.1 Review Service（IngestionService）

管理 package 从提交到入库的完整生命周期。Review Service 的审查流程分为四个阶段：

| 阶段 | 说明 |
|------|------|
| **1. Validation** | Re-compile package source，diff raw graph，校验结构完整性 |
| **2. Canonicalization** | 审计 local canonical graph 的分组决策质量 |
| **3. Multiple Agent Review** | 多个独立 review agent 并行评估推理质量，给出 probability judgments |
| **4. Gatekeeper** | 综合所有 review 结果做最终准入决策：global matching + integrate 或 reject |

#### 状态机

```
submitted → validating → validated
                ↓
              invalid (raw graph mismatch → blocking)

validated → auditing_canonicalization → canonicalization_audited
                     ↓
                  needs_revision (bad merge → blocking)

canonicalization_audited → multi_agent_reviewing → reviewed
                                   ↓
                                needs_revision → (rebuttal cycle, max 5 rounds)
                                   ↓
                                rejected

reviewed → gatekeeper → global_matching → matched → integrating → merged
                                            ↓
...
离线图维护服务，与 BPService 共同构成全局图的定期维护机制。Curation 负责结构层面的维护，BP 负责概率层面的更新。推荐执行顺序：**Curation → BP**——先在结构上清理图（聚类、去重、修复矛盾），再在清洁的图上跑信念传播。
...
class CurationService:
    ...
    async def cluster_similar_conclusions(self) -> list[ClusterResult]
    async def discover_contradictions(self) -> list[ContradictionReport]
    async def inspect_structure(self) -> StructureReport
    async def cleanup(self, plan: CleanupPlan) -> CleanupResult
```
...
| 职责 | 说明 |
|------|------|
| **相似结论聚类** | 发现全局图中语义相似但尚未合并的 GlobalCanonicalNode，建议合并或标记等价关系 |
| **矛盾发掘** | 检测图中隐含的矛盾——两个高置信度结论通过推理链互相否定，但未被显式标记为 contradiction |
| **全图结构巡检** | 检查图的健康状态：孤立节点、悬空 factor、环路异常、度数分布偏斜等 |
| **图清理** | 基于巡检结果执行清理：合并重复节点、移除无效 factor、修复拓扑异常 |

### 5.4 QueryService（Read Side）

Read Side 的核心服务。**一等消费者是 AI agents**，核心用例是科研探索（research）——agents 通过 QueryService 搜索、浏览和探索全局知识图，发现新的研究方向和知识关联。
...
class QueryService:
    ...
    async def get_subgraph(self, node_id: str, layer: str,
...
def create_dependencies() -> Dependencies:
    ...
    curation = CurationService(storage)
    query = QueryService(storage)

    return Dependencies(ingestion, bp, curation, query)
```
...
class StorageManager:
    ...
    async def get_belief_history(self, global_canonical_id: str) -> list[BeliefSnapshot]

    # ── 图查询（委托 GraphStore）──
    async def get_neighbors(self, node_id: str, layer: str, direction: str,
                            max_hops: int) -> Subgraph
    async def get_subgraph(self, node_id: str, layer: str, max_knowledge: int) -> Subgraph
...
| GraphStore 有两层拓扑 | Chain PREMISE/CONCLUSION + GlobalCanonicalNode FACTOR_PREMISE/CONCLUSION/CONTEXT | 向后兼容 authoring-layer 查询；全局 BP 统一跑在 GlobalCanonicalNode 上 |
...
| Write/Read Side 分离 | Domain Services 按写入路径和消费路径组织 | 明确数据流方向，Write Side 聚焦入库+维护，Read Side 聚焦 agent 探索 |
| Curation + BP 离线协作 | Curation 先做结构维护，BP 再跑推理更新 | 在干净的图上跑 BP 效果更好；两者职责正交（结构 vs 概率） |
| Review Service 四步流程 | validation → canonicalization → multiple agent → gatekeeper | 工程化 peer review 流程，multiple agent 提供独立评估，gatekeeper 做最终准入 |
| Read Side agent-first | QueryService 面向 AI agents，核心用例是 research | agents 是全局知识图的主要消费者，human dashboard 是辅助 |
...
8. **CurationService 详细设计** — 相似结论聚类算法、矛盾发掘策略、结构巡检规则、Curation→BP 调度策略
9. **Multiple Agent Review 协议** — 多个 review agent 的注册、并行评审、结果聚合机制
10. **Read Side agent API 设计** — 面向 research agents 的 API 设计，支持探索式查询和知识发现
