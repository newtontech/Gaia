...
`supported_by` 是唯一核心 support API。receiver Claim 是 conclusion；`inputs` 是支持它的 Claim/Warrant；`pattern` 说明这条 warrant 的论证模式；`reason` 是作者给出的理由文本；`label` 给这条 warrant 一个可引用名字。`reason` 对 published Warrant 应该必填；`prior` 是可选作者默认值，reviewer 可以之后覆盖。
...
`Warrant` 是 `Claim` 子类，所以 warrant prior 本质上就是 Warrant claim 的 prior。区别只在 review UI：domain Claim 和 Warrant 分开导出、分开审查。Warrant 的 review item 展示自动生成的 `question`、作者写的 `reason`、以及引用到的 `inputs` / `conclusion`。Strategy 本身不再承载 v6 authoring-level prior；它只连接 `premises=[warrant, *inputs]` 到 conclusion。
...
| `deduction([a], b)` | `b.supported_by(inputs=[a], pattern="deduction", reason=..., prior=0.99)` | deduction 是 Warrant pattern |
