...
| **不确定性建模** | 信念度区间 [lower, upper]——约束满足给出范围而非点值 | 点估计后验概率——BP 给出每个变量的边际 belief；配合 review sidecar 记录不确定性的结构来源 |
...
| **审查机制** | 无 | Review sidecar：每条推理链附带结构化审查报告，审查结论影响先验和策略参数 |
...
5. **Review sidecar：** Gaia 为每条推理链附带结构化审查报告（验证研究设计、数据完整性、推理有效性等维度），审查结论可以调整先验和策略参数。这在 Epistemic Graphs 中没有对应。
...
这与 Gaia 的 **review sidecar** 有弱平行关系。在 Gaia 中，review sidecar 记录审查者对推理链的结构化评估（研究设计 0-5、数据完整性 0-5、推理有效性 0-5 等维度），审查结论可以调整先验和策略参数。不同审查者可能给出不同的评估——这在功能上类似于 Carneades 中不同听众赋予不同权重。
