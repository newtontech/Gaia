...
5. Each `<premise>` and `<context>` element MUST include all of the following attributes:
   - `id` — Sequential identifier (P1, P2, ... for premises; C1, C2, ... for contexts)
   - `conclusion_id` — The conclusion this weak point was identified in
   - `step_id` — The reasoning step where this weak point appears
   - `title` — A short descriptive title
   - `name` — A short, unique, snake_case identifier (lowercase, underscores, max 80 chars) that will be used as the knowledge object name in YAML output. The name should be descriptive and derived from the proposition's core claim. Examples: `weak_coupling_approximation`, `thermal_equilibrium_assumption`, `sample_density_estimate`.

   Additionally, each `<premise>` MUST include:
   - `prior_probability` — A float between 0 and 1

6. **Uniqueness**: Each `name` attribute must be unique across all premises and contexts in the output.
...
<inference_unit>
<premises>
<premise id="P1" conclusion_id="2" step_id="3" prior_probability="0.75" title="..." name="thermal_equilibrium_assumption">
[Self-contained proposition describing the premise]
<ref type="citation">[Full citation if applicable]</ref>
</premise>

<premise id="P2" conclusion_id="4" step_id="7" prior_probability="0.60" title="..." name="perturbative_regime_validity">
[Self-contained proposition describing the premise]
</premise>
...
<contexts>
<context id="C1" conclusion_id="1" step_id="4" title="..." name="auxiliary_background_observation">
