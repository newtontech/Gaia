...
#claim(from: (<eddington_sobral>, <eddington_principe>, <gr_light_deflection>))[
  1919 年日食观测数据支持广义相对论的 $1.75''$ 光线偏折预测，
  排除牛顿微粒说的 $0.87''$ 预测。
][
  Sobral 站测得 $1.98'' plus.minus 0.16''$ @eddington_sobral ，
  与广义相对论预测 @gr_light_deflection 的偏差：
  $ frac(|1.98 - 1.75|, 0.16) approx 1.4 sigma $
  与牛顿预测的偏差：
  $ frac(|1.98 - 0.87|, 0.16) approx 6.9 sigma $
  Príncipe 站测得 $1.61'' plus.minus 0.30''$ @eddington_principe ：
...
#relation(type: "corroboration", between: (<eddington_confirms_gr>, <gr_mercury_precession>))[
  光线偏折与水星进动是广义相对论的两个独立预测，
  分别被不同类型的观测所证实。
][
  爱丁顿日食观测 @eddington_confirms_gr 验证了 GR 对光线偏折的预测：
  $ theta_"obs" approx 1.75'' = frac(4 G M, c^2 R) $
  水星进动 @gr_mercury_precession 验证了 GR 对行星轨道的预测：
