...
#claim(from: (<light_bends_in_gravity>, <einstein_field_equations>))[
  广义相对论预测：光线掠过太阳表面时偏折 $1.75''$。
][
  等价原理已定性预测光线在引力场中弯曲 @light_bends_in_gravity 。
  在 Schwarzschild 度规下 @einstein_field_equations ，
  光线经过质量 $M$ 的天体时偏折角为：
  $ theta = frac(4 G M, c^2 R) $
  其中 $R$ 为光线与天体中心的最近距离。
  这一结果包含两部分贡献：
  $ underbrace(frac(2 G M, c^2 R), "时间弯曲") + underbrace(frac(2 G M, c^2 R), "空间弯曲") = frac(4 G M, c^2 R) $
  Soldner 的牛顿微粒说 @soldner_deflection 仅考虑时间弯曲，
  得到 $theta_"Newton" = 2 G M \/ (c^2 R) = 0.87''$。
  广义相对论的空间弯曲贡献恰好再翻一倍：
  $ theta_"GR" = 2 theta_"Newton" = 1.75'' $
] <gr_light_deflection>

#relation(type: "contradiction", between: (<gr_light_deflection>, <soldner_deflection>))[
  广义相对论预测光线偏折 $1.75''$，牛顿微粒说预测 $0.87''$，
  两者针对同一物理量给出不同数值，不可能同时正确。
][
  广义相对论 @gr_light_deflection 的完整计算给出：
  $ theta_"GR" = frac(4 G M, c^2 R) = 1.75'' $
  Soldner 的牛顿微粒说 @soldner_deflection 仅含时间弯曲项：
...
#claim(from: (<einstein_field_equations>, <mercury_perihelion>))[
  广义相对论精确解释了水星近日点每世纪 $43''$ 的异常进动，
  无需引入任何新参数。
][
  在 Schwarzschild 度规下 @einstein_field_equations 求解行星轨道，
  广义相对论给出每圈额外进动：
