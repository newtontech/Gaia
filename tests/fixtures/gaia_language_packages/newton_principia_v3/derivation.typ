...
#claim("inverse_square_force")[
  ...
  由牛顿第二定律 @second-law ，做匀速圆周运动的物体需要向心力：
  $ F = frac(m_i v^2, r) $
  轨道速度 $v = 2 pi r \/ T$，代入：
  $ F = frac(m_i dot 4 pi^2 r^2, r dot T^2) = frac(4 pi^2 m_i r, T^2) $
  由开普勒第三定律 @kepler-third-law $T^2 = k r^3$，代入消去 $T$：
...
#claim("law_of_gravity")[
  ...
  由反平方关系 @inverse-square-force ，天体 A（质量 $M$）对物体 B（质量 $m$）的引力：
  $ F_(A B) = C(M) dot frac(m, r^2) $
  其中 $C(M)$ 是仅依赖 A 质量的系数。
  由牛顿第三定律 @third-law $F_(A B) = F_(B A)$，从 B 对 A 的视角：
...
#claim("freefall_acceleration_equals_g")[
  ...
  对自由落体，唯一的力是引力。
  联立牛顿第二定律 @second-law 与万有引力定律 @law-of-gravity ：
  $ m_i a = frac(G M m_g, r^2) $
  由质量等价 @mass-equivalence $m_i = m_g$，两边约去质量：
  $ a = frac(G M, r^2) $
  在地球表面 @near-earth-surface $r approx R$：
  $ a = frac(G M, R^2) = g $
...
#claim_relation("galileo_newton_convergence",
  ...
  伽利略的论证路径 @vacuum-prediction ：
  从"重者下落更快"假设出发，通过绑球思想实验揭示逻辑矛盾，
  结合斜面实验与介质消除，推出真空中一切物体等速下落。

  牛顿的论证路径 @freefall-acceleration-equals-g ：
...
#claim_relation("apollo_galileo_convergence",
  type: "corroboration",
  between: ("apollo15_confirms_equal_fall", "vacuum_prediction"),
)[伽利略 1638 年的思想实验预测与 Apollo 15 1971 年的月面实验结果一致：
  在真空中一切物体以相同速率下落。
][
  伽利略通过绑球思想实验与介质消除论证 @vacuum-prediction ，
  在 1638 年从纯逻辑推理出发预测真空中一切物体等速下落——
  但当时无法制造真空来直接验证。

  333 年后，Apollo 15 月面实验 @apollo15-confirms-equal-fall
  在月球的天然真空中直接验证了这一预测：
