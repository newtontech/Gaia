...
#claim(from: (<heavier_falls_faster>, <thought_experiment_env>))[
  假设"重者下落更快"，将重球（$H$）与轻球（$L$）绑成复合体（$H L$），
  则 $H L$ 的下落速度慢于 $H$ 单独下落。
][
  在"重者更快"假设下 @heavier_falls_faster ，
  考虑绑球思想实验 @thought_experiment_env ：
...
#claim(from: (<heavier_falls_faster>, <thought_experiment_env>))[
  ...
  同一假设下 @heavier_falls_faster ，
  在绑球思想实验中 @thought_experiment_env ，
  复合体 $H L$ 总质量 $= H + L > H$，
...
#relation(
  ...
  两个预测来自同一假设"重者下落更快"，
  针对同一物理对象（复合体 $H L$）：
  一方面，$L$ 的拖拽效应要求 $H L$ 比 $H$ 慢 @composite_is_slower ；
  另一方面，$H L$ 总质量更大要求它比 $H$ 快 @composite_is_faster 。
  快与慢不可能同时成立，因此原假设自相矛盾。
] <tied_balls_contradiction>

// -- Medium elimination --
#claim(from: (<medium_density_observation>,))[
  日常观察到的速度差异更应被解释为介质阻力造成的表象，
  而不是重量本身决定自由落体速度的证据。
][
  如果速度差异由介质阻力造成，那么介质越稀薄差异越小。
  实验中恰好观察到了这一规律 @medium_density_observation ，
  说明介质阻力是更好的解释。
] <air_resistance_is_confound>

// -- Final synthesis --
#claim(from: (<tied_balls_contradiction>, <air_resistance_is_confound>, <inclined_plane_observation>))[
  在真空中，不同重量的物体应以相同速率下落。
][
  + 绑球思想实验表明旧定律自相矛盾 @tied_balls_contradiction 。
  + 日常观察到的速度差异实为介质阻力的表象 @air_resistance_is_confound 。
  + 斜面实验从正面提供了等速趋势的证据 @inclined_plane_observation 。
