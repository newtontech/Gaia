...
class TestMakeQid:
    def test_basic(self):
        assert make_qid("github", "my_pkg", "my_label") == "github:my_pkg::my_label"
...
class TestIsQid:
    def test_valid_qid(self):
        assert is_qid("github:my_pkg::my_label")
    ...
    def test_invalid_no_double_colon(self):
        assert not is_qid("github:pkg:label")
...
class TestKnowledgeCreation:
    def test_explicit_qid(self):
        k = Knowledge(id="github:pkg::x", type=KnowledgeType.CLAIM, content="test", label="x")
        assert k.id == "github:pkg::x"
        assert k.label == "x"
    ...
    def test_same_content_same_hash_different_packages(self):
        ...
        k1 = Knowledge(id="github:pkg_a::x", type="claim", content="test", label="x")
        k2 = Knowledge(id="github:pkg_b::y", type="claim", content="test", label="y")
    ...
    def test_different_content_different_hash(self):
        k1 = Knowledge(id="github:pkg::a", type="claim", content="A", label="a")
        k2 = Knowledge(id="github:pkg::b", type="claim", content="B", label="b")
        assert k1.content_hash != k2.content_hash
...
class TestKnowledgeParameters:
    def test_closed_claim_empty_params(self):
        k = Knowledge(id="github:test::k1", type="claim", content="test", parameters=[])
        assert k.parameters == []
    ...
    def test_params_affect_content_hash(self):
        k1 = Knowledge(id="github:pkg::a", type="claim", content="P({x})", label="a")
        k2 = Knowledge(
            id="github:pkg::b",
            type="claim",
            content="P({x})",
...
class TestKnowledgeMetadata:
    def test_metadata_refs(self):
        k = Knowledge(
            ...
            type="claim",
            content="test",
            metadata={"refs": ["github:test::k2", "github:test::k3"]},
        )
        assert k.metadata["refs"] == ["github:test::k2", "github:test::k3"]
