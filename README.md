...
pipeline_review()   Graph IR + LLM → priors, factor parameters
pipeline_infer()    Adapted factor graph → belief propagation → beliefs
pipeline_publish()  → storage (LanceDB + graph DB)
```
